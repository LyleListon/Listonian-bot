...
class AlertSystem:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.anomaly_detection_enabled = self.config.get(
            "anomaly_detection_enabled", True
        )
        self.anomaly_sensitivity = float(
            self.config.get("anomaly_sensitivity", 2.0)
        )  # Standard deviations

        # Predictive alert settings
        self.predictive_alerts_enabled = self.config.get(
            "predictive_alerts_enabled", True
        )
        self.prediction_horizon = int(
            self.config.get("prediction_horizon", 24)
        )  # hours
    ...
    async def check_threshold_alerts(self, metrics: Dict[str, Any]) -> None:
        ...
        for metric_name, metric_value in metrics.items():
            ...
            if "critical" in thresholds and self._check_threshold(
            ...
            elif "error" in thresholds and self._check_threshold(
                metric_value, thresholds["error"]
            ):
                await self.add_alert(
                    alert_type="threshold",
            ...
            elif "warning" in thresholds and self._check_threshold(
            ...
            elif "info" in thresholds and self._check_threshold(
                metric_value, thresholds["info"]
            ):
                await self.add_alert(
                    alert_type="threshold",
    ...
    async def check_anomaly_alerts(
        ...
        if z_score > self.anomaly_sensitivity:
            ...
            await self.add_alert(
                ...
                severity=(
                    "warning" if z_score < self.anomaly_sensitivity * 1.5 else "error"
                ),
                data={
                    "metric_name": metric_name,
    ...
    async def add_predictive_alert(
        ...
        for level in ["critical", "error", "warning", "info"]:
            if level in thresholds and self._check_threshold(
                predicted_value, thresholds[level]
            ):
                severity = level
        ...
        async with self.lock:
            ...
            if start_time:
                filtered_alerts = [
                    ...
                    if a.get("timestamp", datetime.min) >= start_time
                ]
            ...
            if end_time:
                filtered_alerts = [
                    a
                    for a in filtered_alerts
                    if a.get("timestamp", datetime.max) <= end_time
                ]
    ...
    def set_alert_threshold(
        self, metric_name: str, severity: str, operator: str, value: Any
    ) -> None:
        """
