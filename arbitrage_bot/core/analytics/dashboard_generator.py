...
class DashboardGenerator:
    ...
    async def generate_profit_dashboard(self, timeframe: str = "7d") -> Dict[str, Any]:
        ...
        if (
            cache_key in self._cached_components
            and current_time - self._last_cache_update < self.cache_ttl
        ):
        ...
        async with self.lock:
            ...
            profit_time_series = await self.profit_tracker.get_profit_time_series(
                timeframe=timeframe, interval="1h"
            )
            top_token_pairs = await self.profit_tracker.get_top_token_pairs(
                timeframe=timeframe, limit=5
            )
            ...
            dashboard_data = {
                ...
                "chart_data": {
                    ...
                    "token_pair_chart": self._prepare_token_pair_chart_data(
                        top_token_pairs
                    ),
    ...
    def _prepare_profit_chart_data(
        self, time_series_data: Dict[str, List[Any]]
    ) -> Dict[str, Any]:
    ...
    def _prepare_token_pair_chart_data(
        self, token_pairs: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Prepare token pair chart data for visualization."""
        labels = [
            pair.get("token_pair", "").split("_")[0][-6:]
            + "/"
            + pair.get("token_pair", "").split("_")[1][-6:]
    ...
    async def generate_performance_dashboard(
        ...
        async with self.lock:
            ...
            performance_metrics = (
                await self.performance_analyzer.get_performance_metrics(
                    timeframe=timeframe
                )
            )
            drawdown_analysis = await self.performance_analyzer.get_drawdown_analysis(
                timeframe=timeframe
            )
            ...
            try:
                benchmark_comparison = (
                    await self.performance_analyzer.benchmark_performance(
                        benchmark_id="ETH", timeframe=timeframe
                    )
                )
            ...
            dashboard_data = {
                ...
                "chart_data": {
                    "metrics_chart": self._prepare_metrics_chart_data(
                        performance_metrics
                    ),
                    "drawdown_chart": self._prepare_drawdown_chart_data(
                        drawdown_analysis
                    ),
                    "benchmark_chart": (
                        self._prepare_benchmark_chart_data(benchmark_comparison)
    ...
    def _prepare_drawdown_chart_data(self, drawdown: Dict[str, Any]) -> Dict[str, Any]:
        ...
        max_drawdowns = [
            period.get("max_drawdown", 0) * 100 for period in periods
    ...
    def _prepare_benchmark_chart_data(
                ...
                "datasets": [
                    {
                        "label": "Strategy",
                        "data": [
                            benchmark.get("relative_return", 0)
                            * 100,  # Convert to percentage
                    ...
                    },
                    {
                        ...
                        "data": [
                            benchmark.get("benchmark_return", 0)
                            * 100,  # Convert to percentage
    ...
    async def generate_market_dashboard(
        self, tokens: List[str], timeframe: str = "24h"
    ) -> Dict[str, Any]:
        """
        ...
        async with self.lock:
            ...
            for token in tokens:
                try:
                    trend = await self.market_analyzer.detect_market_trend(
                        token_address=token, timeframe=timeframe
                    )
            ...
            if len(tokens) >= 2:
                try:
                    correlation_analysis = (
                        await self.market_analyzer.analyze_token_correlation(
                            token_addresses=tokens, timeframe=timeframe
                        )
                    )
                except Exception as e:
            ...
            for token in tokens:
                try:
                    forecast = await self.market_analyzer.forecast_volatility(
                        token_address=token, timeframe=timeframe
                    )
                    volatility_forecasts[token] = forecast
                except Exception as e:
                    logger.error(
                        f"Error getting volatility forecast for token {token}: {e}"
                    )
            ...
            dashboard_data = {
                ...
                "chart_data": {
                    "trend_charts": {
                        token: self._prepare_trend_chart_data(trend)
                        for token, trend in token_trends.items()
                    },
                    "correlation_chart": (
                        self._prepare_correlation_chart_data(correlation_analysis)
                        if correlation_analysis
                        else None
                    ),
                    "volatility_chart": self._prepare_volatility_chart_data(
                        volatility_forecasts
                    ),
                },
            }
    ...
    def _prepare_correlation_chart_data(
        ...
        labels = [
            token[-6:] for token in tokens
        ]  # Use last 6 characters of token address
        data = []
        ...
        return {
            ...
            "options": {
                ...
                "plugins": {
                    "title": {"display": True, "text": "Token Correlation Matrix"},
                    "tooltip": {
                        "callbacks": {
    ...
    def _prepare_volatility_chart_data(
        ...
        historical_volatility = [
            volatility_data[token].get("historical_volatility", 0) * 100
            for token in tokens
        ]  # Convert to percentage
        forecasted_volatility = [
            volatility_data[token].get("forecasted_volatility", 0) * 100
            for token in tokens
        ]  # Convert to percentage
        ...
        return {
            ...
            "options": {
                ...
                "plugins": {
                    "title": {"display": True, "text": "Token Volatility Comparison"}
                },
            },
        }

    async def export_dashboard_data(
        self, dashboard_type: str, format: str = "json"
    ) -> bytes:
        """
        ...
        if dashboard_type == "profit":
            ...
            dashboard_data = await self.generate_market_dashboard(
                tokens=tokens, timeframe="7d"
            )
        else:
