...
class ProfitTracker:
    ...
    async def _save_historical_data(self) -> None:
        ...
        try:
            async with self.lock:
                ...
                logger.info(f"Saved {len(data_to_save)} profit entries to {self.profit_file}")
        except Exception as e:
    ...
    async def get_profit_by_token_pair(self, token_pair: Optional[str] = None, 
        ...
        if timeframe not in self._timeframe_profits:
            raise ValueError(f"Invalid timeframe: {timeframe}. Valid options: {list(self._timeframe_profits.keys())}")
        ...
        async with self.lock:
            ...
            if token_pair:
                ...
                entries = [e for e in entries if e.get('token_pair') == token_pair or 
                          (e.get('token_in') and e.get('token_out') and 
                           f"{e['token_in']}_{e['token_out']}" == token_pair)]
                ...
                return {
                    ...
                    'profit_per_volume': total_profit / total_volume if total_volume > 0 else 0
                }
            else:
                ...
                for pair, pair_entries in token_pair_entries.items():
                    ...
                    pair_volume = sum(float(e.get('amount_in', 0)) for e in pair_entries)
                    ...
                    result['token_pairs'][pair] = {
                        ...
                        'profit_per_volume': pair_profit / pair_volume if pair_volume > 0 else 0,
                        'percentage_of_total': pair_profit / result['total_profit'] * 100 if result['total_profit'] > 0 else 0
                    }
                
                # Add average metrics
                if result['trade_count'] > 0:
                    result['avg_profit'] = result['total_profit'] / result['trade_count']
                else:
                    result['avg_profit'] = 0
                
                if result['total_volume'] > 0:
                    result['profit_per_volume'] = result['total_profit'] / result['total_volume']
                else:
    ...
    async def get_roi(self, timeframe: str = "24h") -> Dict[str, Any]:
        ...
        async with self.lock:
            ...
            annualized_roi = roi * days_factor.get(timeframe, 1) if timeframe != "all" else None
    ...
    async def get_profit_time_series(self, timeframe: str = "7d", 
                                    interval: str = "1h") -> Dict[str, List[Any]]:
        """
        ...
        if interval not in interval_minutes:
            raise ValueError(f"Invalid interval: {interval}. Valid options: {list(interval_minutes.keys())}")
        ...
        async with self.lock:
            ...
            if timeframe == "1h":
                ...
                if self._profit_history:
                    ...
                    start_time = now - timedelta(days=30)  # Default to 30 days if no data
            
            # Filter entries by timeframe
            entries = [e for e in self._profit_history if e.get('timestamp', now) >= start_time]
            ...
            for start, end in intervals:
                interval_entries = [e for e in entries if start <= e.get('timestamp', now) < end]
                
                interval_profit = sum(float(e.get('profit', 0)) for e in interval_entries)
                interval_volume = sum(float(e.get('amount_in', 0)) for e in interval_entries)
    ...
    async def get_top_token_pairs(self, timeframe: str = "7d", limit: int = 10) -> List[Dict[str, Any]]:
        """
    ...
    def _get_first_trade_date(self) -> Optional[str]:
        ...
        timestamps = [e.get('timestamp') for e in self._profit_history if e.get('timestamp')]
        if not timestamps:
            return None
    ...
    async def _get_most_profitable_day(self) -> Dict[str, Any]:
        ...
        return {
            'date': most_profitable_date[0],
            'profit': most_profitable_date[1]
        }
...
async def create_profit_tracker(config: Optional[Dict[str, Any]] = None) -> ProfitTracker:
    """
