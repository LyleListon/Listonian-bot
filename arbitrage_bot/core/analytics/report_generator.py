...
class ReportGenerator:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.report_formats = self.config.get('report_formats', ['json', 'csv'])
    ...
    async def _generate_monthly_report(self, month: Optional[str] = None) -> Dict[str, Any]:
        ...
        if self.profit_tracker:
            try:
                ...
                profit_data = await self.profit_tracker.get_profit_by_token_pair(timeframe="30d")
                roi_data = await self.profit_tracker.get_roi(timeframe="30d")
        ...
        if self.performance_analyzer:
            try:
                # Get performance metrics
                performance_metrics = await self.performance_analyzer.get_performance_metrics(timeframe="30d")
                drawdown_analysis = await self.performance_analyzer.get_drawdown_analysis(timeframe="30d")
        ...
        if self.trading_journal:
            try:
                # Get trade analysis
                trade_analysis = await self.trading_journal.analyze_trade_outcomes(timeframe="30d")
                ...
                # Get learning insights
                insights = await self.trading_journal.extract_learning_insights(timeframe="30d")
                if insights:
