...
class TradingJournal:
    ...
    async def log_trade(self, trade_data: Dict[str, Any]) -> None:
        ...
        required_fields = ['token_in', 'token_out', 'amount_in', 'amount_out', 'profit', 'gas_cost']
        ...
        # Add trade outcome
        net_profit = float(trade_data.get('profit', 0)) - float(trade_data.get('gas_cost', 0))
        ...
        if 'gas_cost' in trade_data and float(trade_data['gas_cost']) > 0:
            gas_to_profit_ratio = float(trade_data['gas_cost']) / float(trade_data['profit']) if float(trade_data['profit']) > 0 else float('inf')
    ...
    async def get_trades(self, 
        ...
        async with self.lock:
            ...
            if token_pair:
                filtered_trades = [
                    trade for trade in filtered_trades
                    if (trade.get('token_in') and trade.get('token_out') and
                        f"{trade['token_in']}_{trade['token_out']}" == token_pair)
                ]
            ...
            filtered_trades.sort(key=lambda x: x.get('timestamp', datetime.min), reverse=True)
    ...
    async def analyze_trade_outcomes(self, timeframe: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        async with self.lock:
            ...
            # Calculate metrics
            successful_trades = [trade for trade in trades if trade.get('success', False)]
            failed_trades = [trade for trade in trades if not trade.get('success', False)]
            
            total_profit = sum(float(trade.get('net_profit', 0)) for trade in successful_trades)
            total_loss = abs(sum(float(trade.get('net_profit', 0)) for trade in failed_trades))
            net_profit = total_profit - total_loss
            
            avg_profit = net_profit / len(trades) if trades else 0
            avg_gas_cost = sum(float(trade.get('gas_cost', 0)) for trade in trades) / len(trades) if trades else 0
            
            # Find most and least profitable trades
            if trades:
                most_profitable = max(trades, key=lambda x: float(x.get('net_profit', 0)))
                least_profitable = min(trades, key=lambda x: float(x.get('net_profit', 0)))
            else:
    ...
    async def extract_learning_insights(self, timeframe: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        async with self.lock:
            ...
            unprofitable_trades = [t for t in trades if float(t.get('net_profit', 0)) <= 0]
            
            if profitable_trades and unprofitable_trades:
                # Compare gas costs
                avg_gas_profitable = sum(float(t.get('gas_cost', 0)) for t in profitable_trades) / len(profitable_trades)
                avg_gas_unprofitable = sum(float(t.get('gas_cost', 0)) for t in unprofitable_trades) / len(unprofitable_trades)
                ...
                if avg_gas_profitable < avg_gas_unprofitable:
                    insights.append({
                        ...
                        'data': {
                            ...
                            'difference_percent': (avg_gas_unprofitable - avg_gas_profitable) / avg_gas_unprofitable * 100 if avg_gas_unprofitable > 0 else 0
                        }
                    })
                ...
                for dex, count in dexes_profitable.items():
                    ...
                    unprofitable_ratio = dexes_unprofitable.get(dex, 0) / len(unprofitable_trades) if len(unprofitable_trades) > 0 else 0
                    
                    if profitable_ratio > unprofitable_ratio * 1.5:  # 50% more common in profitable trades
                ...
                if profitable_dexes:
                    insights.append({
                        ...
                        'data': {
                            'profitable_dexes': profitable_dexes
                        }
                    })
            ...
            for pair, data in token_pairs.items():
                if data['count'] >= 5:  # At least 5 trades
                    ...
                    avg_profit = data['total_profit'] / data['profitable'] if data['profitable'] > 0 else 0
            ...
            high_gas_trades = [t for t in trades if float(t.get('gas_cost', 0)) > 0 and float(t.get('net_profit', 0)) <= 0]
            if high_gas_trades:
                improvement_areas.append({
                    ...
                    'data': {
                        ...
                        'avg_gas_cost': sum(float(t.get('gas_cost', 0)) for t in high_gas_trades) / len(high_gas_trades),
                        'total_gas_spent': sum(float(t.get('gas_cost', 0)) for t in high_gas_trades)
                    }
                })
            ...
            if unprofitable_trades:
                ...
                for dex, count in dex_counts.items():
                    if count / len(unprofitable_trades) > 0.5:  # Present in more than 50% of unprofitable trades
...
async def create_trading_journal(config: Optional[Dict[str, Any]] = None) -> TradingJournal:
    """
