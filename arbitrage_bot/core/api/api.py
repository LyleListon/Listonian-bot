    ...
    def _register_routes(self):
        ...
        async def login(username: str, password: str) -> Token:
            ...
            try:
                ...
                token = self._create_token(
                    {"sub": username, "role": "user"}  # Get from your user store
                )
        ...
        async def create_api_key(
            ...
            try:
                ...
                return APIKey(key=key, name=request.name, expires_at=expires_at)
        ...
        async def list_api_keys(
            ...
            try:
                ...
                # Get user's API keys
                keys = [
                    APIKey(key=key, name=data["name"], expires_at=data["expires_at"])
        ...
        async def delete_api_key(
            key: str, token: HTTPAuthorizationCredentials = Security(self.security)
        ):
            """Delete API key."""
            try:
                # Verify JWT
