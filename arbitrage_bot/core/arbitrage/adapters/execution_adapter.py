...
class ArbitrageExecutorAdapter(ExecutionStrategy):
        ...
        **kwargs
        ...
        try:
            ...
            if not result or not result.get("success", False):
                ...
                error_message = result.get("error", "Unknown execution error") if result else "No execution result"
    ...
    def _convert_to_execution_data(self, opportunity: ArbitrageOpportunity) -> Dict[str, Any]:
        ...
        try:
            ...
            while time.time() < end_time:
                try:
                    ...
                    tx_receipt = await self.web3_manager.get_transaction_receipt(transaction_hash)
                    ...
                    if tx_receipt:
                        ...
                        if status == 1:
                            ...
                            if not result.actual_profit and kwargs.get("calculate_profit", False):
                                result.actual_profit = await self._calculate_profit_from_receipt(
                                    tx_receipt, 
                                    kwargs.get("opportunity")
                                )
                            
                            logger.info(f"Transaction {transaction_hash} confirmed successfully")
                            return result
                        else:
                            # Transaction failed
                            ...
                            logger.warning(f"Transaction {transaction_hash} failed on-chain")
                            return result
            ...
            result.error_message = f"Transaction monitoring timed out after {timeout_seconds} seconds"
...
async def create_execution_adapter(
    ...
    if arbitrage_executor is None:
        if flash_loan_manager is None or web3_manager is None:
            raise ValueError("Either arbitrage_executor or both flash_loan_manager and web3_manager must be provided")
...
async def create_transaction_monitor_adapter(
    ...
    """
    return TransactionMonitorAdapter(
        web3_manager=web3_manager,
        config=config
    )