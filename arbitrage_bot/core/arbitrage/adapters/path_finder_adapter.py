...
class PathFinderDetector(OpportunityDetector):
    ...
    def __init__(
        self, path_finder: PathFinder, web3_manager: Any, config: Dict[str, Any] = None
    ):
        """
        ...
        self.base_tokens = self.config.get(
            "base_tokens",
            [
    ...
    async def detect_opportunities(
        self, market_condition: MarketCondition, max_results: int = 10, **kwargs
        ...
        try:
            ...
            sorted_opportunities = sorted(
                all_opportunities, key=lambda x: x.expected_profit, reverse=True
            )
    ...
    async def _convert_to_opportunity(
        self, path_data: Dict[str, Any], market_condition: MarketCondition
    ) -> ArbitrageOpportunity:
    ...
    def _calculate_confidence_score(
        self, profit_margin: float, market_condition: MarketCondition, input_token: str
    ) -> float:
        ...
        if token_lower in market_condition.liquidity_levels:
            liquidity_factor = (
                0.3 + 0.7 * market_condition.liquidity_levels[token_lower]
            )

        # Calculate final confidence
        # Profit is 50%, market conditions are 50% of overall confidence
        market_confidence = (
            congestion_factor * 0.3 + volatility_factor * 0.3 + liquidity_factor * 0.4
        ) * 0.5
    ...
    async def validate_opportunity(
        ...
        **kwargs,
        ...
        try:
            ...
            ) / max(1, opportunity.expected_profit)
            if profit_deviation > 0.1:  # More than 10% deviation
                logger.warning(
                    f"Profit deviation too high for opportunity {opportunity.id}: {profit_deviation:.2%}"
                )
                opportunity.status = OpportunityStatus.REJECTED
                return opportunity

            # Update gas estimate based on simulation
            gas_used = simulation_result.get(
                "gas_used", opportunity.gas_estimate.gas_limit
            )
            opportunity.gas_estimate.gas_limit = gas_used
            opportunity.gas_estimate.total_cost_wei = (
                gas_used * opportunity.gas_estimate.gas_price
            )
            ...
            if profit_wei <= gas_cost_wei * 1.2:  # Require 20% margin over gas cost
                logger.warning(
                    f"Insufficient profit margin over gas cost for opportunity {opportunity.id}"
                )
                opportunity.status = OpportunityStatus.REJECTED
                return opportunity
            ...
            if profit_wei < min_profit_wei:
                logger.warning(
                    f"Profit below threshold for opportunity {opportunity.id}"
                )
                opportunity.status = OpportunityStatus.REJECTED
                return opportunity
            ...
            opportunity.metadata["state_changes"] = simulation_result.get(
                "state_changes", []
            )
...
async def create_path_finder_detector(
    ...
    if path_finder is None:
        ...
        path_finder = await create_path_finder(
            dex_manager=dex_manager, web3_manager=web3_manager, config=config
        )

    # Create detector
    detector = PathFinderDetector(
        path_finder=path_finder, web3_manager=web3_manager, config=config
    )
