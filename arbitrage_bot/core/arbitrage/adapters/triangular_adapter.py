...
class TriangularArbitrageDetector(OpportunityDetector):
    ...
    async def detect_opportunities(
        ...
        try:
            ...
            eth_price_usd = (
                market_condition.eth_price_usd or 3000.0
            ...
            logger.info(
                f"Found {len(opportunities)} triangular arbitrage opportunities"
            )
    ...
    def _convert_to_opportunity(
        self, result: Dict[str, Any], eth_price_usd: float
    ) -> ArbitrageOpportunity:
        ...
        if steps:
            ...
            route = ArbitrageRoute(
                steps=[], input_token_address="", output_token_address=""
            )
    ...
    def _adjust_for_market_conditions(
        self, opportunity: ArbitrageOpportunity, market_condition: MarketCondition
    ) -> None:
        ...
        if input_token in market_condition.liquidity_levels:
            liquidity_factor = (
                0.3 + 0.7 * market_condition.liquidity_levels[input_token]
            )

        # Calculate final confidence
        final_confidence = (
            base_confidence * congestion_factor * volatility_factor * liquidity_factor
        )
...
async def create_triangular_arbitrage_detector(
    dex_manager: Any, web3_manager: Any, config: Dict[str, Any] = None
    ...
    triangular_arbitrage = TriangularArbitrage(
        config=config or {}, dex_interfaces=dex_manager.get_dex_interfaces()
    )
