...
class AnalyticsManager(ArbitrageAnalytics):
    ...
    def __init__(self, storage_dir: Path = Path("memory-bank")):
        ...
        self._memory_bank = MemoryBank(storage_dir=storage_dir)
    ...
    async def get_performance_metrics(
        ...
        async with self._lock:
            ...
            daily_stats = await self._memory_bank.get_daily_stats(days=time_period_days)
    ...
    async def get_recent_opportunities(
        ...
        async with self._lock:
            ...
            trades = await self._memory_bank.get_recent_trades(max_results=max_results)
            # Filter pending trades
            return [
                t
                for t in trades
                if t["status"] == "PENDING"
                and float(t["expected_profit_wei"]) / 10**18 >= min_profit_eth
            ]
