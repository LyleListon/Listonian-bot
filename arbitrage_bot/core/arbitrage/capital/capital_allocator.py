...
class CapitalAllocator:
    ...
    def _calculate_kelly_allocations(
        self, paths: List[ArbitragePath], available_capital: Decimal
    ) -> List[Decimal]:
        """
        ...
        try:
            ...
            for path in paths:
                ...
                if odds > 0:
                    kelly = (
                        win_probability * odds - (Decimal("1") - win_probability)
                    ) / odds
    ...
    def _calculate_expected_profit(
        self, paths: List[ArbitragePath], allocations: List[Decimal]
    ) -> Decimal:
        """
        ...
        try:
            ...
            for i, (path, allocation) in enumerate(zip(paths, allocations)):
                ...
                if (
                    path.optimal_amount is not None
                    and path.optimal_amount > 0
                    and path.profit is not None
                ):
        ...
        try:
            ...
            for i, (path, allocation) in enumerate(zip(paths, allocations)):
                ...
                if len(self._last_allocations[path_id]) > 100:
                    self._last_allocations[path_id] = self._last_allocations[path_id][
                        -100:
                    ]
