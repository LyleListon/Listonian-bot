...
class PortfolioOptimizer:
        ...
        async with self._lock:
            try:
                ...
                result = {
                    "success": True,
                    "allocations": {
                        str(i): float(a) for i, a in enumerate(allocations)
                    },
                    "portfolio_metrics": {
    ...
    def _calculate_simple_allocations(
        self, opportunities: List[MultiPathOpportunity], total_capital: Decimal
    ) -> List[Decimal]:
        """
        Calculate simple allocations based on expected profit.
        ...
        try:
            ...
            total_profit = sum(
                opp.expected_profit for opp in opportunities if opp.expected_profit > 0
            )
            ...
            weights = [
                (
                    opp.expected_profit / total_profit
            ...
            if total_weight > 0:
                ...
                normalized_weights = [Decimal("1") / Decimal(len(opportunities))] * len(
                    opportunities
                )
    ...
    def _calculate_simple_metrics(
        self, opportunities: List[MultiPathOpportunity], allocations: List[Decimal]
    ) -> Dict[str, Decimal]:
        """
        ...
        try:
            if (
                not opportunities
                or not allocations
                or len(opportunities) != len(allocations)
            ):
                return {"expected_return": Decimal("0"), "sharpe_ratio": Decimal("0")}
            ...
            avg_confidence = sum(opp.confidence_level for opp in opportunities) / len(
                opportunities
            )
            ...
            return {"expected_return": expected_return, "sharpe_ratio": sharpe_ratio}
