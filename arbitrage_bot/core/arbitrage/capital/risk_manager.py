...
class RiskManager:
        ...
        async with self._lock:
            try:
                ...
                risk_adjusted_return = self._calculate_risk_adjusted_return(
                    path, allocation, max_loss
                )
                sharpe_ratio = self._calculate_sharpe_ratio(path, context)

                # Calculate risk exposure
                token_exposure = self._calculate_token_exposure(
                    path, allocation, total_capital
                )
                dex_exposure = self._calculate_dex_exposure(
                    path, allocation, total_capital
                )
                ...
                assessment = {
                    ...
                    "token_exposure": {
                        token: float(exposure)
                        for token, exposure in token_exposure.items()
                    },
                    "dex_exposure": {
                        dex: float(exposure) for dex, exposure in dex_exposure.items()
                    },
    ...
    async def assess_opportunity_risk(
        ...
        async with self._lock:
            try:
                ...
                risk_acceptable = combined_max_loss <= (
                    total_capital * self.max_risk_per_trade
                )
                ...
                assessment = {
                    ...
                    "diversification_benefit": float(
                        combined_max_loss - correlation_adjusted_risk
                    ),
                }
    ...
    async def size_position(
        ...
        try:
            ...
            max_position = self._calculate_max_position_size(
                path, total_capital, context
            )
    ...
    async def update_risk_metrics(
        self, trade_result: Dict[str, Any], current_capital: Decimal
    ) -> None:
        """
    ...
    def _calculate_max_loss(self, path: ArbitragePath, allocation: Decimal) -> Decimal:
        """
    ...
    def _calculate_risk_adjusted_return(
        self, path: ArbitragePath, allocation: Decimal, max_loss: Decimal
    ) -> Decimal:
        """
    ...
    def _calculate_token_exposure(
        self, path: ArbitragePath, allocation: Decimal, total_capital: Decimal
    ) -> Dict[str, Decimal]:
        """
    ...
    def _calculate_max_position_size(
        self, path: ArbitragePath, total_capital: Decimal, context: Dict[str, Any]
    ) -> Decimal:
        """
        ...
        try:
            ...
            if self._current_drawdown > Decimal("0"):
                drawdown_factor = Decimal("1") - (
                    self._current_drawdown / self.max_drawdown
                )
    ...
    def _calculate_combined_risk_score(
        self, path_assessments: List[Dict[str, Any]]
    ) -> Decimal:
        """
    ...
    def _group_correlated_paths(self, paths: List[ArbitragePath]) -> List[List[int]]:
        """
    ...
    def _calculate_path_correlation(
        ...
        try:
            ...
            correlation = token_overlap * 0.4 + pool_overlap * 0.4 + dex_overlap * 0.2
    ...
    def _update_risk_exposure(self, trade_result: Dict[str, Any]) -> None:
        """
