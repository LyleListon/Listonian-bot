...
from ..models import ArbitrageOpportunity, TokenAmount, StrategyType
...
class DefaultDiscoveryManager(DiscoveryManager):
    ...
    def __init__(
        self, market_data_provider: MarketDataProvider, config: Dict[str, Any] = None
    ):
        """
        ...
        default_config = {
            ...
            "preferred_flash_loan_providers": [
                "balancer",
                "aave",
            ],  # Preferred flash loan providers
            # MEV protection
        ...
        **kwargs,
        ...
        if self.config.get("parallel_detection", True):
            ...
            for detector_id, detector in self._detectors.items():
                tasks.append(
                    self._run_detector(
                        detector, detector_id, market_condition, semaphore
                    )
                )
        ...
        validated_opportunities.sort(key=lambda x: x.expected_profit_wei, reverse=True)
    ...
    async def _validate_opportunity(
        self, opportunity: ArbitrageOpportunity, market_condition: Dict[str, Any]
    ) -> bool:
        """
        ...
        for validator_id, validator in self._validators.items():
            try:
                is_valid, error_message, confidence = (
                    await validator.validate_opportunity(opportunity, market_condition)
                )

                if not is_valid:
                    logger.debug(
                        f"Opportunity {opportunity.id} rejected by validator {validator_id}: {error_message}"
                    )
                    return False

                # Update confidence score
                opportunity.confidence_score = min(
                    opportunity.confidence_score, confidence or 1.0
                )
    ...
    def _select_simple_strategy(
        ...
        """
        return {"type": StrategyType.SIMPLE, "params": {}}

    def _select_flash_loan_strategy(
        self, opportunity: ArbitrageOpportunity
    ) -> Dict[str, Any]:
        """
        ...
        Returns:
            Strategy configuration
        """
        preferred_providers = self.config.get(
            "preferred_flash_loan_providers", ["balancer", "aave"]
        )
    ...
    def _select_multi_path_strategy(
        self, opportunity: ArbitrageOpportunity
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "params": {"max_path_length": max_path_length, "paths": opportunity.path},
        }

    def _select_custom_strategy(
        self, opportunity: ArbitrageOpportunity
    ) -> Dict[str, Any]:
        """
