...
class DefiLlamaSource(DEXSource):
        ...
        try:
            async with self._session.get(url) as response:
                if response.status != 200:
                    logger.error(f"Error fetching protocols from DeFiLlama: {response.status}")
                    return []
    ...
    async def _fetch_dexes(self) -> Dict[str, Any]:
        ...
        try:
            async with self._session.get(url) as response:
                if response.status != 200:
                    logger.error(f"Error fetching DEXes from DeFiLlama: {response.status}")
                    return {}
...
async def create_defillama_source(config: Optional[Dict[str, Any]] = None) -> DefiLlamaSource:
    """
