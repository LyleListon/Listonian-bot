...
class DexScreenerSource(DEXSource):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # API configuration
        self._base_url = self.config.get("base_url", "https://api.dexscreener.com/latest")
        ...
        try:
            async with self._session.get(url) as response:
                if response.status != 200:
                    logger.error(f"Error fetching DEXes from DexScreener: {response.status}")
                    return []
    ...
    async def _fetch_top_pairs(self, chain: str, dex: str) -> List[Dict[str, Any]]:
        ...
        try:
            async with self._session.get(url) as response:
                if response.status != 200:
                    logger.error(f"Error fetching pairs from DexScreener: {response.status}")
                    return []
    ...
    def _extract_addresses(self, pairs: List[Dict[str, Any]]) -> Dict[str, str]:
        ...
        for pair in pairs:
            ...
            if "factory" in pair and not addresses.get("factory"):
                try:
                    addresses["factory"] = to_checksum_address(pair["factory"]["address"])
                except Exception:
                    pass
...
async def create_dexscreener_source(config: Optional[Dict[str, Any]] = None) -> DexScreenerSource:
    """
