...
class DEXRepository:
    ...
    async def get_dexes_by_protocol(self, protocol_type: DEXProtocolType) -> List[DEXInfo]:
        """
        ...
        async with self._lock:
            return [dex for dex in self._dexes.values() if dex.protocol_type == protocol_type]
    ...
    async def mark_validated(self, name: str, validated: bool = True, errors: List[str] = None) -> bool:
        """
    ...
    async def _load_dexes(self) -> None:
        ...
        if not os.path.exists(storage_path):
            logger.info(f"DEX storage file {storage_path} not found, starting with empty repository")
            return
...
async def create_dex_repository(config: Optional[Dict[str, Any]] = None) -> DEXRepository:
    """
