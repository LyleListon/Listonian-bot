...
class DEXValidator:
    ...
    def __init__(self, web3_manager: Any, config: Optional[Dict[str, Any]] = None):
        """
    ...
    async def validate_dex(self, dex_info: DEXInfo) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            protocol_valid, protocol_errors = await self._validate_protocol_type(dex_info)
    ...
    async def _validate_factory(self, dex_info: DEXInfo) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            if not self.web3_manager.w3.is_address(dex_info.factory_address):
                errors.append(f"Invalid factory address: {dex_info.factory_address}")
                return False, errors
            
            # Check if contract exists
            code = await self.web3_manager.w3.eth.get_code(dex_info.factory_address)
            if code == b'' or code == '0x':
                errors.append(f"Factory contract does not exist: {dex_info.factory_address}")
                return False, errors
            ...
            contract = self.web3_manager.contract(dex_info.factory_address, [])
            
            # Check if contract has required functions
            for func_name, func_sig in signatures.items():
                if not await self._has_function(contract, func_sig):
    ...
    async def _validate_router(self, dex_info: DEXInfo) -> Tuple[bool, List[str]]:
        ...
        try:
            # Check if address is valid
            if not self.web3_manager.w3.is_address(dex_info.router_address):
                errors.append(f"Invalid router address: {dex_info.router_address}")
                return False, errors
            
            # Check if contract exists
            code = await self.web3_manager.w3.eth.get_code(dex_info.router_address)
            if code == b'' or code == '0x':
                errors.append(f"Router contract does not exist: {dex_info.router_address}")
                return False, errors
            ...
            # Create contract instance
            contract = self.web3_manager.contract(dex_info.router_address, [])
    ...
    async def _validate_quoter(self, dex_info: DEXInfo) -> Tuple[bool, List[str]]:
        ...
        try:
            # Check if address is valid
            if not self.web3_manager.w3.is_address(dex_info.quoter_address):
                errors.append(f"Invalid quoter address: {dex_info.quoter_address}")
                return False, errors
            
            # Check if contract exists
            code = await self.web3_manager.w3.eth.get_code(dex_info.quoter_address)
            if code == b'' or code == '0x':
                errors.append(f"Quoter contract does not exist: {dex_info.quoter_address}")
                return False, errors
    ...
    async def _validate_protocol_type(self, dex_info: DEXInfo) -> Tuple[bool, List[str]]:
        """
        Validate protocol type.
        ...
        try:
            ...
            factory_contract = self.web3_manager.contract(dex_info.factory_address, [])
            
            # Check for V2-specific functions
            is_v2 = await self._has_function(factory_contract, self._function_signatures["factory_v2"]["getPair(address,address)"])
            
            # Check for V3-specific functions
            is_v3 = await self._has_function(factory_contract, self._function_signatures["factory_v3"]["getPool(address,address,uint24)"])
            
            # Validate protocol type
            if dex_info.protocol_type == DEXProtocolType.UNISWAP_V2 and not is_v2:
                errors.append(f"DEX {dex_info.name} is marked as Uniswap V2 but factory does not have V2 functions")
            elif dex_info.protocol_type == DEXProtocolType.UNISWAP_V3 and not is_v3:
                errors.append(f"DEX {dex_info.name} is marked as Uniswap V3 but factory does not have V3 functions")
    ...
    async def _has_function(self, contract: Any, function_signature: str) -> bool:
        """
        Check if a contract has a function with the given signature.
        ...
        Returns:
            True if the contract has the function, False otherwise
        """
        try:
            # Get contract code
            code = await self.web3_manager.w3.eth.get_code(contract.address)
            
            # Check if function signature is in code
            return function_signature in code.hex()
...
async def create_dex_validator(web3_manager: Any, config: Optional[Dict[str, Any]] = None) -> DEXValidator:
    """
