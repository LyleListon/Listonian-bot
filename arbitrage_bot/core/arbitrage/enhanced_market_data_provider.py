...
class EnhancedMarketDataProvider(MarketDataProviderABC):
        ...
        async with self._lock:
            ...
            try:
                ...
                self._dex_discovery_manager = await create_dex_discovery_manager(
                    web3_manager=self._web3, config=dex_discovery_config
                )
                logger.info("DEX discovery manager initialized")
            except Exception as e:
                logger.error(
                    f"Failed to initialize DEX discovery manager: {e}", exc_info=True
                )
                logger.warning(
                    "Continuing without DEX discovery - will use only configured DEXes"
                )
    ...
    async def cleanup(self) -> None:
        ...
        async with self._lock:
            ...
            try:
                ...
                logger.error(
                    f"Error cleaning up DEX discovery manager: {e}", exc_info=True
                )
    ...
    async def start_monitoring(
        self, update_interval_seconds: Optional[float] = None
    ) -> None:
        """
    ...
    async def _fetch_dex_price(
        ...
        for attempt in range(self._retry_count):
            try:
                ...
                version = (
                    dex_config.get("version", "v3") if dex_config else "v3"
                ...
                if attempt == self._retry_count - 1:
                    logger.debug(
                        f"Failed to fetch {dex_name} price after {self._retry_count} attempts: {e}"
                    )
                    return None
                await asyncio.sleep(
    ...
    async def _fetch_initial_market_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            dex_configs = {
                k: v for k, v in self._config["dexes"].items() if v is not None
            }

            # BaseSwap V3
            baseswap_config = dex_configs.get("baseswap")  # Use correct key
            logger.debug(
                f"Initial fetch - BaseSwap config retrieved: {baseswap_config}"
            )
                ...
                if baseswap_price is not None:
                    ...
                    try:
                        liquidity["baseswap_v3"] = await self._web3.get_pool_liquidity(
                            baseswap_config.get("factory"), weth_address, usdc_address
                        )
                    except Exception as liq_error:
                        logger.warning(
                            f"Failed to get BaseSwap V3 liquidity: {liq_error}"
                        )
                        liquidity["baseswap_v3"] = 0  # Default liquidity if fetch fails
                    self._price_cache["baseswap_v3"] = baseswap_price
            else:
                logger.info(
                    "BaseSwap V3 is disabled in config, skipping initial fetch."
                )

            # Aerodrome V3
            aerodrome_config = dex_configs.get("aerodrome")  # Use correct key
            logger.debug(
                f"Initial fetch - Aerodrome config retrieved: {aerodrome_config}"
            )
                ...
                if aerodrome_price is not None:
                    ...
                    try:
                        liquidity["aerodrome_v3"] = await self._web3.get_pool_liquidity(
                            aerodrome_config.get("factory"), weth_address, usdc_address
                        )
                    except Exception as liq_error:
                        logger.warning(
                            f"Failed to get Aerodrome V3 liquidity: {liq_error}"
                        )
                        liquidity["aerodrome_v3"] = (
                            0  # Default liquidity if fetch fails
                        )
                    self._price_cache["aerodrome_v3"] = aerodrome_price
            else:
                logger.info(
                    "Aerodrome V3 is disabled in config, skipping initial fetch."
                )
            ...
            if self._dex_discovery_manager:
                try:
                    ...
                    logger.info(
                        "Discovering additional DEXes for initial market data..."
                    )
                    discovered_dexes = (
                        await self._dex_discovery_manager.discover_dexes()
                    )
                    logger.info(
                        f"Discovered {len(discovered_dexes)} additional DEXes for initial market data"
                    )

                    # Filter valid DEXes
                    valid_dexes = [dex for dex in discovered_dexes if dex.validated]
                    logger.info(
                        f"Found {len(valid_dexes)} valid additional DEXes for initial market data"
                    )
                    ...
                    for dex in valid_dexes:
                        ...
                        if dex_key in prices:
                            logger.debug(
                                f"Skipping discovered DEX {dex.name} as it's already processed"
                            )
                            continue
                        ...
                        if dex_price is not None:
                            ...
                            try:
                                liquidity[dex_key] = (
                                    await self._web3.get_pool_liquidity(
                                        dex.factory_address, weth_address, usdc_address
                                    )
                                )
                            except Exception as liq_error:
                                logger.warning(
                                    f"Failed to get {dex.name} liquidity: {liq_error}"
                                )
                                liquidity[dex_key] = (
                                    0  # Default liquidity if fetch fails
                                )
                    ...
                    logger.error(
                        f"Error discovering additional DEXes: {e}", exc_info=True
                    )

            # Get initial analysis
            market_data = {
                "token_pair": (weth_address, usdc_address),
                "liquidity": liquidity,
            }
            analysis = await self._analyzer.analyze_market_data(
                market_data, list(prices.values()), list(prices.keys())
            )
    ...
    async def _fetch_market_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if baseswap_config and baseswap_config.get("enabled", False):
                ...
                logger.debug(
                    "BaseSwap V3 is disabled in config, skipping update fetch."
                )

            # Aerodrome V3
            aerodrome_config = dex_configs.get("aerodrome")  # Use correct key
            logger.debug(
                f"Update fetch - Aerodrome config retrieved: {aerodrome_config}"
            )
            if aerodrome_config and aerodrome_config.get("enabled", False):
                aerodrome_price = await self._fetch_dex_price(
                    "Aerodrome V3",
                ...
                logger.debug(
                    "Aerodrome V3 is disabled in config, skipping update fetch."
                )
            ...
            if self._dex_discovery_manager:
                try:
                    ...
                    logger.debug(
                        "Getting additional DEXes from repository for market data update..."
                    )
                    discovered_dexes = await self._dex_discovery_manager.get_dexes()
                    logger.debug(
                        f"Found {len(discovered_dexes)} additional DEXes in repository for market data update"
                    )

                    # Filter valid DEXes
                    valid_dexes = [dex for dex in discovered_dexes if dex.validated]
                    logger.debug(
                        f"Found {len(valid_dexes)} valid additional DEXes for market data update"
                    )
