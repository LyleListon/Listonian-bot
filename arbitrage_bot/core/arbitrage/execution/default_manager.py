    ...
    async def execute_opportunity(
        ...
        **kwargs,
        ...
        if opportunity.status != OpportunityStatus.VALIDATED:
            logger.warning(
                f"Opportunity {opportunity.id} is not validated, skipping execution"
            )
        ...
        async with self._execution_lock:
            ...
            min_time_between_executions = int(
                self.config.get("min_time_between_executions", 0)
            )
            ...
            execution_task = asyncio.create_task(
                self._execute_opportunity_internal(
                    opportunity, market_condition, **kwargs
                )
            )
    ...
    async def _execute_opportunity_internal(
        ...
        **kwargs,
        ...
        if strategy_name in self.strategies:
            ...
            logger.warning(
                f"Strategy {strategy_name} not found, using default strategy"
            )
        ...
        try:
            ...
            logger.info(
                f"Executing opportunity {opportunity.id} with strategy {strategy_name}"
            )
            ...
            if (
                ...
                and execution_result.status == ExecutionStatus.SUBMITTED
                and execution_result.transaction_info is not None
            ):
                transaction_hash = execution_result.transaction_info.transaction_hash

                # Monitor transaction
                logger.info(
                    f"Monitoring transaction {transaction_hash} for opportunity {opportunity.id}"
                )
                updated_transaction_info = (
                    await self.transaction_monitor.monitor_transaction(
                ...
                if updated_transaction_info:
                    ...
                    execution_result.total_cost = updated_transaction_info.gas_used * (
                        updated_transaction_info.gas_price
                        + updated_transaction_info.priority_fee
                    )
    ...
    async def cancel_execution(self, opportunity_id: str) -> bool:
        ...
        async with self._execution_lock:
            if opportunity_id in self._active_executions:
                ...
                logger.warning(
                    f"Opportunity {opportunity_id} not found in active executions"
                )
                return False
