...
class GasOptimizer:
        ...
        async with self._lock:
            try:
                if not opportunity.paths:
                    return {"success": False, "error": "No paths in opportunity"}
    ...
    async def predict_gas_price(
        ...
        async with self._lock:
            try:
                ...
                confidence_interval = self._calculate_gas_price_confidence_interval(
                    predicted_gas_price
                )
    ...
    async def _update_gas_price(self) -> None:
        ...
        try:
            ...
            if (
                current_time - self._last_gas_price_update
                >= self.gas_price_update_interval
            ):
                ...
                if len(self._gas_price_history) > self.gas_price_history_window:
                    self._gas_price_history = self._gas_price_history[
                        -self.gas_price_history_window :
                    ]
        ...
        try:
            ...
            for i, path in enumerate(opportunity.paths):
                ...
                if path_result["success"]:
                    ...
                    # Add to path gas
                    path_gas.append(
                        {"path_index": i, "gas": path_result["buffered_gas"]}
                    )
    ...
    def _predict_gas_price_from_history(self, time_horizon: int) -> float:
        """
        Predict gas price based on historical data.
    ...
    def _calculate_gas_price_confidence_interval(
        self, predicted_gas_price: float
    ) -> Dict[str, float]:
        """
        ...
        try:
            ...
            std_dev = (
                np.std(gas_prices) if len(gas_prices) > 1 else predicted_gas_price * 0.1
            )
    ...
    def _calculate_optimal_priority_fee(
        self, gas_price: float, base_fee: float, optimization_target: str
    ) -> float:
        """
        ...
        try:
            ...
            priority_fee = max(
                self.min_priority_fee, min(self.max_priority_fee, priority_fee)
            )
    ...
    def _calculate_total_gas_cost(self, optimization_result: Dict[str, Any]) -> float:
        """
