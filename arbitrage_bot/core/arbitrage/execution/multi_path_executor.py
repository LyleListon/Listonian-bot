...
class MultiPathExecutor:
        ...
        try:
            ...
            if execution_strategy == 'atomic':
                ...
                for i, (path, allocation) in enumerate(zip(opportunity.paths, opportunity.allocations)):
                    path_txs = await self._create_path_transactions(path, allocation, context)
    ...
    def _validate_opportunity(
        ...
        try:
            ...
            if not opportunity.is_valid:
                return {
                    'valid': False,
                    'reason': 'Opportunity is not valid'
                }
            
            # Check if opportunity has expired
            if opportunity.expiration is not None and opportunity.expiration < int(time.time()):
                return {
                    'valid': False,
            ...
            if opportunity.expected_profit <= 0:
                return {
                    'valid': False,
                    'reason': 'Opportunity has no expected profit'
                }
    ...
    async def _execute_atomic(
        ...
        try:
            ...
            if self.simulation_manager:
                ...
                if simulation_results.get('mevValue', 0) <= simulation_results.get('totalCost', 0):
                    return {
                        'success': False,
            ...
            if not inclusion_result['success'] and context.get('fallback_enabled', True):
    ...
    async def _execute_sequential(
        ...
        try:
            ...
            for i, (path, allocation) in enumerate(zip(opportunity.paths, opportunity.allocations)):
                ...
                if not path_result['success'] and not context.get('fallback_enabled', True):
                    break
    ...
    async def _execute_parallel(
        ...
        try:
            ...
            for i, (path, allocation) in enumerate(zip(opportunity.paths, opportunity.allocations)):
                ...
                task = asyncio.create_task(self._execute_path(path, allocation, context))
                tasks.append(task)
    ...
    async def _execute_path(
        ...
        try:
            ...
            transactions = await self._create_path_transactions(path, allocation, context)
    ...
    async def _wait_for_bundle_inclusion(
        self,
        bundle_hash: str
    ) -> Dict[str, Any]:
        """
