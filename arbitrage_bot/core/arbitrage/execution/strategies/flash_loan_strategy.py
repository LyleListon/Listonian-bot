...
class FlashLoanExecutionStrategy(ExecutionStrategy):
        ...
        try:
            ...
            for token_name, token_address in common_tokens.items():
                try:
                    ...
                    logger.warning(f"Failed to pre-initialize provider for {token_name}: {e}")
    ...
    async def get_estimated_cost(
        ...
        try:
            ...
            logger.info(f"Estimated flash loan fee: {min_fee} (provider: {min_fee_provider})")
    ...
    async def is_profitable(
        ...
        try:
            ...
            logger.info(f"Opportunity profitability: expected={expected_profit}, "
        ...
        async with self._execution_lock:
            ...
            try:
                ...
                if not await self.is_profitable(opportunity):
                    logger.warning(f"Opportunity {opportunity.id} is not profitable after costs")
                ...
                try:
                    ...
                    # Execute flash loan
                    flash_loan_result = await provider.execute_flash_loan(params, callback)
                    
                    # Check result
                    if flash_loan_result.success:
                        primary_success = True
                        logger.info(f"Successfully executed with primary provider {provider.name}")
                    else:
                        logger.warning(f"Failed with primary provider {provider.name}: "
                ...
                if not primary_success:
                    ...
                    try:
                        ...
                        provider_type = (
                            "aave" if "Balancer" in self._cached_providers.get(
                        ...
                        logger.info(f"Executing with fallback provider: {provider.name}")
                        ...
                        if await provider.check_liquidity(token_address, amount):
                            ...
                            # Check result
                            if flash_loan_result.success:
                                logger.info(f"Successfully executed with fallback provider {provider.name}")
                            else:
                                logger.warning(f"Failed with fallback provider {provider.name}: "
                                             f"{flash_loan_result.error_message}")
                        else:
                            logger.warning(f"Fallback provider {provider.name} has insufficient liquidity")
                ...
                if callback.success:
                    ...
                    result.transaction_hash = getattr(flash_loan_result, "transaction_hash", None)
