...
class FlashbotsFlashLoanStrategy(ExecutionStrategy):
    ...
    def __init__(
        ...
        # Parameters
        self.slippage_tolerance = Decimal(self.flash_loan_config.get("slippage_tolerance", "0.005"))
        self.profit_threshold_multiplier = Decimal(self.flash_loan_config.get("profit_threshold_multiplier", "1.5"))
    ...
    async def is_profitable(self, opportunity: ArbitrageOpportunity) -> bool:
        ...
        try:
            ...
            if not provider:
                logger.info(f"No flash loan provider available for {opportunity.start_token}")
                return False
    ...
    async def execute(
        ...
        try:
            ...
            if not provider:
                logger.error(f"No flash loan provider available for {opportunity.start_token}")
            ...
            bundle = await self._create_flashbots_bundle(flash_loan_tx, execution_params)
            ...
            is_profitable = await self.bundle_simulator.verify_profitability(
                simulation_result,
                min_profit_threshold=float(execution_params.get("min_profit_threshold", 0.001))
            )
    ...
    async def _calculate_actual_profit(
        ...
        try:
            ...
            # Subtract gas cost
            gas_cost = Decimal(stats.get("gas_used", 0)) * Decimal(stats.get("gas_price", 0)) / Decimal(10**18)
