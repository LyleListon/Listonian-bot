...
class FlashbotsExecutionStrategy(ExecutionStrategy):
    ...
    def __init__(
        ...
        # Configuration
        self.flashbots_endpoint = self.config.get("flashbots_endpoint", "https://relay.flashbots.net")
        ...
        self.inclusion_timeout = float(self.config.get("inclusion_timeout", 60.0))  # 60 seconds
        self.block_wait_time = float(self.config.get("block_wait_time", 12.0))  # ~12 seconds per block
        ...
        async with self._execution_lock:
            ...
            try:
                ...
                if bundle_result.status == ExecutionStatus.SUCCESS:
                    ...
                    logger.warning(f"Opportunity {opportunity.id} execution aborted: {bundle_result.error_message}")
                else:
                    self._execution_stats["failed"] += 1
                    error = bundle_result.error_message or "Unknown error"
                    self._execution_stats["errors"][error] = self._execution_stats["errors"].get(error, 0) + 1
                    logger.error(f"Opportunity {opportunity.id} execution failed: {error}")
                ...
                self._execution_stats["failed"] += 1
                self._execution_stats["errors"][str(e)] = self._execution_stats["errors"].get(str(e), 0) + 1
    ...
    async def _execute_with_flashbots(
        ...
        if not transactions:
            ...
            result.error_message = "No valid transactions could be built from opportunity steps"
            return result
        ...
        if self.simulation_required:
            ...
            if not simulation_result or not simulation_result.get("success", False):
                error = simulation_result.get("error", "Unknown simulation error") if simulation_result else "Simulation failed"
                result.status = ExecutionStatus.ABORTED
                result.error_message = f"Simulation error: {error}"
                return result
            
            # Check profitability
            if not simulation_result.get("profitable", False) and self.min_profit_wei > 0:
                profit = simulation_result.get("profit_wei", 0)
                result.status = ExecutionStatus.ABORTED
                result.error_message = f"Not profitable: {profit} wei < {self.min_profit_wei} wei minimum"
                return result
        ...
        inclusion_success = await bundle.wait_for_inclusion(
            ...
            timeout=self.inclusion_timeout
        )
        
        if not inclusion_success:
            result.status = ExecutionStatus.FAILED
            result.error_message = f"Bundle not included within {self.inclusion_timeout} seconds"
            return result
    ...
    async def _build_transactions_from_steps(
        ...
        for step in steps:
            try:
                ...
                logger.error(f"Error building transaction from step {step.step_id}: {e}")
