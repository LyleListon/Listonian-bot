...
class MultiPathStrategy(ExecutionStrategy):
    ...
    def __init__(
        ...
        self.profit_threshold_multiplier = Decimal(
            self.config.get("profit_threshold_multiplier", "1.5")
        )
    ...
    async def initialize(self) -> bool:
        ...
        async with self._initialization_lock:
            ...
            try:
                ...
                if (
                    self.flash_loan_factory
                    and not await self.flash_loan_factory.is_initialized()
                ):
                ...
                self.bundle_simulator = BundleSimulator(
                    self.web3_client, config=self.config.get("simulator", {})
                )
        ...
        try:
            ...
            if not self.use_flash_loans:
                balance = await self._get_token_balance(
                    opportunity.start_token, await self._get_account_address()
                )
    ...
    async def execute(
        ...
        try:
            logger.info(
                f"Executing multi-path opportunity with {len(opportunity.paths)} paths"
            )
            ...
            is_profitable = await self.bundle_simulator.verify_profitability(
                simulation_result, min_profit_threshold=min_profit_threshold
            )
    ...
    async def _prepare_flash_loan_execution(
        self, opportunity: MultiPathOpportunity, execution_params: Dict[str, Any]
    ...
    async def _create_flashbots_bundle(
        self, transactions: List[Transaction], execution_params: Dict[str, Any]
        ...
        try:
            ...
            bundle = FlashbotsBundle(
                transactions=transactions, blocks_into_future=blocks_into_future
            )
    ...
    async def _calculate_costs(
        ...
        try:
            ...
            costs["slippage_cost"] = (
                opportunity.required_amount * self.slippage_tolerance
            )

            # Calculate total cost
            costs["total_cost"] = sum(
                cost for key, cost in costs.items() if key != "total_cost"
            )
