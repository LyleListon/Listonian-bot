...
from ....dex.interfaces import DEX, Router
...
class StandardExecutionStrategy(ExecutionStrategy):
    ...
    def __init__(
        ...
        # Configuration
        self.slippage_percentage = Decimal(self.config.get("slippage_percentage", "0.5"))  # 0.5%
        self.gas_limit_buffer = Decimal(self.config.get("gas_limit_buffer", "20"))  # 20% buffer on gas limit
        self.transaction_timeout = int(self.config.get("transaction_timeout", 60))  # 60 seconds
        self.max_retries = int(self.config.get("max_retries", 1))  # Maximum number of retries
        self.default_gas_limit = int(self.config.get("default_gas_limit", 500000))  # Default gas limit
    ...
    async def execute(
        ...
        **kwargs
        ...
        account_address = kwargs.get("account_address", web3_client.get_default_account())
        ...
        async with self._execution_lock:
            try:
                ...
                if not transaction:
                    logger.error(f"Failed to build transaction for opportunity {opportunity.id}")
                    return ExecutionResult(
                        opportunity_id=opportunity.id,
                        status=ExecutionStatus.FAILED,
                        timestamp=int(time.time()),
                ...
                # Submit transaction
                if use_flashbots:
                    logger.info(f"Submitting transaction for opportunity {opportunity.id} using Flashbots")
                    tx_hash = await web3_client.send_flashbots_transaction(transaction)
                else:
                    logger.info(f"Submitting transaction for opportunity {opportunity.id}")
                    tx_hash = await web3_client.send_transaction(transaction)
                
                if not tx_hash:
                    logger.error(f"Failed to submit transaction for opportunity {opportunity.id}")
                    return ExecutionResult(
                        opportunity_id=opportunity.id,
                        status=ExecutionStatus.FAILED,
                        timestamp=int(time.time()),
                ...
                if wait_for_receipt:
                    ...
                    if receipt:
                        ...
                        transaction_info.status = receipt.status
                        transaction_info.gas_used = receipt.gas_used
                        transaction_info.effective_gas_price = receipt.effective_gas_price
                        ...
                        if receipt.status:
                            ...
                            gas_cost = receipt.gas_used * receipt.effective_gas_price
                            ...
                            return ExecutionResult(
                                ...
                                profit=None,
                                gas_used=receipt.gas_used,
                                total_cost=receipt.gas_used * receipt.effective_gas_price
                            )
                
                # If not waiting for receipt, return submitted status
                return ExecutionResult(
                    opportunity_id=opportunity.id,
    ...
    async def _build_transaction(
        ...
        **kwargs
        ...
        try:
            ...
            elif len(opportunity.route.steps) == 2 and opportunity.route.steps[0].dex.id != opportunity.route.steps[1].dex.id:
            ...
            elif all(step.dex.id == opportunity.route.steps[0].dex.id for step in opportunity.route.steps):
            ...
            logger.error(f"Error building transaction for opportunity {opportunity.id}: {e}")
            return None
    ...
    async def _build_single_swap_transaction(
        ...
        **kwargs
        ...
        try:
            ...
            min_output_amount = int(expected_output_amount * (100 - slippage_percentage) / 100)
            
            # Get deadline timestamp
            deadline_seconds = kwargs.get("deadline_seconds", 60 * 20)  # 20 minutes default
    ...
    async def _estimate_gas(
        ...
        try:
            ...
            if gas_estimate:
                ...
                logger.warning(f"Failed to estimate gas, using default: {self.default_gas_limit}")
