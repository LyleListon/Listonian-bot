...
class TransactionMonitor(TransactionMonitorInterface):
    ...
    def __init__(
        ...
        self.max_attempts = int(self.config.get("max_attempts", 30))  # 30 attempts (60 seconds with default poll)
        self.confirmation_blocks = int(self.config.get("confirmation_blocks", 1))  # Number of blocks to consider confirmed
        ...
        **kwargs
        ...
        async with self._transaction_lock:
            if transaction_hash in self._monitored_transactions:
                logger.info(f"Transaction {transaction_hash} is already being monitored")
            ...
            if transaction_hash not in self._monitoring_tasks or self._monitoring_tasks[transaction_hash].done():
        ...
        try:
            ...
            await asyncio.wait_for(
                self._monitoring_tasks[transaction_hash],
                timeout=timeout
            )
        ...
        try:
            ...
            while attempts < self.max_attempts:
                try:
                    ...
                    receipt = await self.web3_client.get_transaction_receipt(transaction_hash)
                    ...
                    if receipt:
                        ...
                        if not wait_for_confirmations:
                            logger.info(f"Transaction {transaction_hash} received receipt and not waiting for confirmations")
                            return
                        ...
                        logger.debug(f"Transaction {transaction_hash} not mined yet, attempt {attempts+1}/{self.max_attempts}")
                        attempts += 1
                        await asyncio.sleep(self.poll_interval)
                
                except Exception as e:
                    logger.error(f"Error checking transaction {transaction_hash} status: {e}")
                    attempts += 1
                    await asyncio.sleep(self.poll_interval)
            
            # If we get here, we've exceeded max attempts
            logger.warning(f"Transaction {transaction_hash} monitoring exceeded max attempts")
            ...
            logger.error(f"Error in monitor_transaction_task for {transaction_hash}: {e}")
    ...
    async def _update_transaction_info(
        ...
        async with self._transaction_lock:
            ...
            if not receipt.status:
                ...
                logger.info(f"Transaction {transaction_hash} succeeded in block {receipt.block_number}")
