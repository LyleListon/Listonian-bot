...
async def create_arbitrage_system(
    web3_client: Web3Client, config: Optional[Dict[str, Any]] = None
) -> ArbitrageSystem:
    """
    ...
    # Create components
    discovery_manager = await create_discovery_manager(
        web3_client, config.get("discovery")
    )
    execution_manager = await create_execution_manager(
        web3_client, config.get("execution")
    )
    analytics_manager = await create_analytics_manager(
        web3_client, config.get("analytics")
    )
    market_data_provider = await create_market_data_provider(
        web3_client, config.get("market_data")
    )
...
async def create_discovery_manager(
    ...
    if config.get("enable_cross_dex_detection", True):
        cross_dex_detector = await create_cross_dex_detector(
            web3_client, config.get("cross_dex_detector")
        )
    ...
    if config.get("enable_triangular_detection", True):
        triangular_detector = await create_triangular_detector(
            web3_client, config.get("triangular_detector")
        )
    ...
    basic_validator = await create_basic_validator(
        web3_client, config.get("basic_validator")
    )
    validators.append(basic_validator)

    # Create manager
    manager = DefaultDiscoveryManager(
        detectors=detectors, validators=validators, config=config
    )
...
async def create_execution_manager(
    ...
    if config.get("enable_standard_strategy", True):
        standard_strategy = await create_standard_strategy(
            web3_client, config.get("standard_strategy")
        )
    ...
    if config.get("enable_flashbots_strategy", True):
        flashbots_strategy = await create_flashbots_strategy(
            web3_client, config.get("flashbots_strategy")
        )
    ...
    if config.get("enable_flash_loan_strategy", True):
        flash_loan_strategy = await create_flash_loan_strategy(
            web3_client, config.get("flash_loan_strategy")
        )
        strategies["flash_loan"] = flash_loan_strategy

    # Create transaction monitor
    transaction_monitor = TransactionMonitor(
        web3_client=web3_client, config=config.get("transaction_monitor")
    )
...
async def create_cross_dex_detector(
    ...
    detector = CrossDexDetector(web3_client=web3_client, config=config)
...
async def create_triangular_detector(
    ...
    detector = TriangularDetector(web3_client=web3_client, config=config)
...
async def create_basic_validator(
    ...
    validator = BasicValidator(web3_client=web3_client, config=config)
...
async def create_standard_strategy(
    ...
    strategy = StandardExecutionStrategy(web3_client=web3_client, config=config)
...
async def create_flashbots_strategy(
    ...
    strategy = FlashbotsExecutionStrategy(
        web3_client=web3_client, flashbots_provider=provider, config=config
    )
...
async def create_flash_loan_strategy(
    ...
    # Create strategy
    strategy = FlashLoanExecutionStrategy(web3_client=web3_client, config=config)
