    ...
    async def find_paths(
        ...
        async with self._lock:
            try:
                ...
                logger.info(
                    f"Finding cycles for {start_token} with max_hops={max_hops}"
                )

                cycles = await self.graph_explorer.find_cycles(
                    start_token=start_token,
                    max_length=max_hops
                    + 1,  # +1 because cycles include start token twice
                ...
                # Cache paths
                self._path_cache[cache_key] = {"timestamp": time.time(), "paths": paths}
        ...
        try:
            ...
            for token in start_tokens:
                task = self.find_paths(
                    start_token=token, max_paths=max_paths_per_token, filters=filters
                )
                tasks.append(task)

            # Execute tasks in parallel with concurrency limit
            results = await gather_with_concurrency(self.concurrency_limit, *tasks)
