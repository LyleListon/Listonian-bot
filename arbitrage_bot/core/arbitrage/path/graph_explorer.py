...
class NetworkXGraphExplorer(GraphExplorer):
    ...
    def __init__(
        ...
        self.min_liquidity = Decimal(
            self.config.get("min_liquidity", "1000")
    ...
    async def initialize(self) -> bool:
        ...
        async with self._initialization_lock:
            ...
            try:
                ...
                logger.info(
                    f"Graph explorer initialized with {len(self.graph.nodes)} tokens and {len(self.graph.edges)} pools"
                )
                return True
    ...
    async def update_graph(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Using {len(filtered_pools)} out of {len(all_pools)} pools after filtering"
            )
            ...
            logger.info(
                f"Graph updated with {len(self.graph.nodes)} tokens and {len(self.graph.edges)} edges"
            )
    ...
    async def find_paths(
        ...
        if start_token not in graph or end_token not in graph:
            logger.warning(
                f"One or both tokens not found in graph: {start_token}, {end_token}"
            )
            return []
        ...
        try:
            ...
            paths: Iterable = nx.all_simple_paths(
                graph, source=start_token, target=end_token, cutoff=max_length
            )
            ...
            for path in paths:
                ...
                for i in range(len(path) - 1):
                    ...
                    if not edge_data:
                        logger.warning(
                            f"Missing edge data for {token_from} -> {token_to}"
                        )
                        continue
    ...
    async def find_cycles(
        ...
        try:
            ...
            cycles = self._find_cycles_from_node(
                graph, start_token, max_length, max_cycles
            )
    ...
    def _filter_pools(self, pools: List[Pool]) -> List[Pool]:
        ...
        for pool in pools:
            ...
            if (
                pool.token0 in self.excluded_tokens
                or pool.token1 in self.excluded_tokens
            ):
                continue
            ...
            if self.included_tokens and (
                pool.token0 not in self.included_tokens
                and pool.token1 not in self.included_tokens
            ):
                continue
    ...
    def _apply_filters_to_graph(
        self, graph: nx.DiGraph, filters: Dict[str, Any]
    ) -> nx.DiGraph:
        """
    ...
    def _find_cycles_from_node(
        self, graph: nx.DiGraph, start_node: str, max_length: int, max_cycles: int
    ) -> List[List[Tuple[str, Pool]]]:
        """
