...
class MultiPathOptimizer:
    ...
    def _select_best_paths(
        self, paths: List[ArbitragePath], max_paths: int
    ) -> List[ArbitragePath]:
        """
        ...
        try:
            ...
            sorted_paths = sorted(paths, key=lambda p: p.expected_profit, reverse=True)
    ...
    def _calculate_slippage_factor(
        self, path: ArbitragePath, allocation: Decimal, token_address: ChecksumAddress
    ) -> Decimal:
        """
    ...
    async def _create_path_transactions(
        self, path: ArbitragePath, allocation: Decimal
    ) -> List[Dict[str, Any]]:
        """
