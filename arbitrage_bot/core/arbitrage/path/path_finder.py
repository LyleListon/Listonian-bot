...
class MultiPathFinder(PathFinder):
        ...
        async with self._initialization_lock:
            ...
            try:
                ...
                if not isinstance(self.graph_explorer, NetworkXGraphExplorer):
                    logger.error(
                        "Graph explorer is not a NetworkXGraphExplorer instance"
                    )
                    return False
    ...
    async def find_paths(
        ...
        try:
            ...
            for path in paths:
                ...
                if (
                    evaluated_path
                    and evaluated_path.profit
                    and evaluated_path.profit > 0
                ):
    ...
    async def evaluate_path(self, path: ArbitragePath) -> ArbitragePath:
        ...
        try:
            ...
            optimal_amount, expected_output, confidence = (
                await self._calculate_optimal_amount(path)
            )

            if optimal_amount <= 0 or expected_output <= optimal_amount:
                logger.debug(
                    f"Path is not profitable: input={optimal_amount}, output={expected_output}"
                )
                return path
    ...
    async def find_opportunities(
        ...
        try:
            ...
            paths = await self.find_paths(
                start_token=start_token, max_paths=self.max_paths, filters=filters
            )
            ...
            filtered_paths = [
                p for p in paths if p.path_yield >= float(min_profit_percentage)
            ]
            ...
            for i, group in enumerate(grouped_paths):
                ...
                allocations, expected_profit = await self.optimizer.optimize_allocation(
                    paths=group, total_capital=capital_available
                )
    ...
    async def _cycle_to_path(
        self, cycle: List[Tuple[str, Pool]]
        ...
        try:
            ...
            for i, pool in enumerate(pools):
                ...
                try:
                    data = await dex.get_swap_data(
                        token_from=token_from, token_to=token_to, pool=pool
                    )
            ...
            return ArbitragePath(
                tokens=tokens, pools=pools, dexes=dexes, swap_data=swap_data
            )
    ...
    async def _calculate_optimal_amount(
        ...
        for amount in test_amounts:
            try:
                ...
                for i, (token_from, token_to) in enumerate(
                    zip(path.tokens[:-1], path.tokens[1:])
                ):
                    pool = path.pools[i]
                    dex = await self.dex_manager.get_dex(pool.dex)
                    ...
                    if not quote_result or quote_result.get("amount_out", 0) <= 0:
                        logger.warning(
                            f"Failed to get quote for {token_from} -> {token_to}"
                        )
                        current_amount = Decimal("0")
                        break
    ...
    async def _estimate_gas_cost(self, path: ArbitragePath) -> Tuple[int, Decimal]:
        """
    ...
    async def _get_available_capital(self, token_address: str) -> Decimal:
        """
    ...
    def _group_paths(self, paths: List[ArbitragePath]) -> List[List[ArbitragePath]]:
        """
