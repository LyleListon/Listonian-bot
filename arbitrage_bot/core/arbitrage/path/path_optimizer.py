...
class PathOptimizer:
    ...
    async def optimize_paths(
        ...
        try:
            ...
            if len(optimized_paths) > 1:
                optimized_paths = await self._merge_similar_paths(optimized_paths, context)
    ...
    async def _optimize_gas_usage(
        ...
        try:
            ...
            gas_cost_eth = Decimal(str(buffered_gas * gas_price)) / Decimal('1000000000')
    ...
    async def _predict_slippage(
        ...
        try:
            ...
            for i, pool in enumerate(path.pools):
                ...
                hop_slippage = self._calculate_hop_slippage(pool, token_in, token_out, amount)
    ...
    async def _adjust_optimal_amount(
        ...
        try:
            ...
            if 'metadata' in path.__dict__ and 'predicted_slippage' in path.__dict__['metadata']:
    ...
    def _group_by_similarity(
        ...
        try:
            ...
            similarity_threshold = 0.7  # Paths with similarity > 0.7 are considered similar
            groups = []
            visited = set()
    ...
    def _calculate_path_similarity(
        ...
        try:
            ...
            if path1.start_token != path2.start_token or path1.end_token != path2.end_token:
                return 0.0
    ...
    def _is_merging_beneficial(
        ...
        try:
            ...
            total_profit = sum(
                path.profit for path in paths
                if path.profit is not None
            )
            
            # Calculate total gas cost of individual paths
            total_gas_cost = sum(
                path.estimated_gas_cost for path in paths
