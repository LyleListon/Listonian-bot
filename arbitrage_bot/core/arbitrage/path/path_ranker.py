    ...
    async def rank_paths(
        ...
        async with self._lock:
            try:
                ...
                ranked_paths = [p for _, p in sorted(
                    zip(path_scores, paths),
                    key=lambda x: x[0],
                    reverse=True
                )]
        ...
        async with self._lock:
            try:
                ...
                if len(self._path_history[path_id]) > self.history_window:
                    self._path_history[path_id] = self._path_history[path_id][-self.history_window:]
                ...
                for token in path.tokens:
                    ...
                    if len(self._token_history[token]) > self.history_window:
                        self._token_history[token] = self._token_history[token][-self.history_window:]
                ...
                for dex in path.dexes:
                    ...
                    if len(self._dex_history[dex]) > self.history_window:
                        self._dex_history[dex] = self._dex_history[dex][-self.history_window:]
    ...
    def _calculate_history_score(
        ...
        try:
            ...
            if path_history:
                ...
                path_score = sum(entry['performance_score'] for entry in path_history) / len(path_history)
            else:
            ...
            for token in path.tokens:
                ...
                if token_history:
                    # Calculate average performance score
                    token_score = sum(entry['performance_score'] for entry in token_history) / len(token_history)
            ...
            for dex in path.dexes:
                ...
                if dex_history:
                    # Calculate average performance score
                    dex_score = sum(entry['performance_score'] for entry in dex_history) / len(dex_history)
