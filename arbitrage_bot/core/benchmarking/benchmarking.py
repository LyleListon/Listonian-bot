...
class BenchmarkingSystem:
    ...
    def calculate_performance_metrics(
        ...
        sortino = (
            ...
            / np.std(excess_returns[excess_returns < 0])
            * np.sqrt(252)
        )
        ...
        return {
            ...
            "profit_factor": (
                ...
                if np.sum(strategy_returns[strategy_returns < 0]) != 0
                else np.inf
            ),
        }
    ...
    async def analyze_strategy(
        ...
        try:
            ...
            filtered_points = [
                p for p in price_points if start_ts <= p.timestamp <= end_ts
            ]
        ...
        try:
            ...
            for name, returns in strategies.items():
                result = await self.analyze_strategy(
                    symbol, start_date, end_date, returns
                )
                results[name] = result
    ...
    async def analyze_market_impact(
        self, symbol: str, trades: List[Dict[str, Any]], window_size: int = 10
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for trade in trades:
                ...
                before_points = [
                    ...
                    if before_start <= p.timestamp <= entry_time.timestamp()
                ]
                after_points = [
                    p
                    for p in price_points
                    ...
                    <= p.timestamp
                    <= (exit_time + timedelta(minutes=window_size)).timestamp()
                ]
