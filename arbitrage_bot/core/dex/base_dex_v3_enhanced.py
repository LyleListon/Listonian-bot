...
class BaseDEXV3(BaseDEX):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if self.quoter_address:
                ...
                self.logger.debug(f"Initialized {self.name} V3 with quoter at {self.quoter_address}")
    ...
    async def get_token_decimals(self, token_address: str) -> int:
        ...
        try:
            ...
            token_contract = await self.web3_manager.get_contract(address=token_address, abi_name="ERC20")
            contract_func = token_contract.functions.decimals()
            decimals = await self.web3_manager.call_contract_function(contract_func)
            return decimals
        except Exception as e:
            self.logger.warning(f"Failed to get token decimals for {token_address}: {e}")
    ...
    async def _get_pool_address_with_fee(self, token0: str, token1: str, fee: int) -> str:
        """
        ...
        try:
            ...
            sorted_tokens = sorted([Web3.to_checksum_address(token0), Web3.to_checksum_address(token1)])
            ...
            self.logger.debug(f"Failed to get pool address for {token0}/{token1} with fee {fee}: {e}")
            return "0x0000000000000000000000000000000000000000"
    ...
    async def _get_pool_liquidity_by_tokens(self, token0: str, token1: str, fee: int = None) -> Decimal:
        """
        ...
        try:
            ...
            self.logger.error(f"Failed to get pool liquidity for {token0}/{token1}: {e}")
            return Decimal(0)
    ...
    async def find_best_path(self, token_in: str, token_out: str, amount_in: int) -> Tuple[List[str], List[int], int]:
        """
        ...
        try:
            ...
            for fee in self.supported_fees:
                ...
                # Get quote
                output = await self._get_quote_with_fee(token_in, token_out, amount_in, fee)
            ...
            for mid_token in common_tokens:
                ...
                if mid_token.lower() == token_in.lower() or mid_token.lower() == token_out.lower():
                    continue
                ...
                for fee1 in self.supported_fees:
                    ...
                    for fee2 in self.supported_fees:
                        ...
                        mid_amount = await self._get_quote_with_fee(token_in, mid_token, amount_in, fee1)
                        if not mid_amount or mid_amount == 0:
                            continue
                            
                        out_amount = await self._get_quote_with_fee(mid_token, token_out, mid_amount, fee2)
                        ...
                        if out_amount > best_output:
                            ...
                            best_fees = [fee1, fee2]
                            self.logger.debug(f"Found better 2-hop path: {best_path} with fees {best_fees}, output: {best_output}")
            
            return best_path, best_fees, best_output
            
        except Exception as e:
            self.logger.error(f"Error finding best path: {e}")
            return [token_in, token_out], [self.fee if not isinstance(self.fee, list) else self.fee[0]], 0
    ...
    async def get_multi_hop_gas_estimate(self, path: List[str], fees: Optional[List[int]] = None) -> int:
        """
    ...
    async def _get_common_tokens(self) -> List[str]:
        ...
        if not common_tokens:
            ...
            common_tokens = [
                ...
                self.config.get('tokens', {}).get('USDC', {}).get('address', '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913'),  # USDC
                self.config.get('tokens', {}).get('USDT', {}).get('address', '0x4D15a3a2286D883AF0AA1B3f21367843FAc63E07'),  # USDT
                self.config.get('tokens', {}).get('DAI', {}).get('address', '0x50c5725949A6F0c72E6C4a641F24049A917DB0Cb')   # DAI
            ]
            
        # Filter out None or empty addresses
        return [token for token in common_tokens if token and token != '0x0000000000000000000000000000000000000000']
    
    async def _get_quote_with_fee(self, token_in: str, token_out: str, amount_in: int, fee: int) -> Optional[int]:
        """
        ...
        try:
            ...
            if hasattr(self.quoter.functions, 'quoteExactInputSingle'):
                ...
                return quote_result if not isinstance(quote_result, (list, tuple)) else quote_result[0]
        except Exception as e:
            self.logger.debug(f"Failed to get quote for {token_in}/{token_out} with fee {fee}: {e}")
            return None