...
class BaseSwapV3(BaseDEXV3):
    ...
    async def get_quote_from_quoter(
        ...
        try:
            ...
            if len(path) > 2:
                self.logger.debug(
                    "Getting multi-hop quote for path with %d tokens", len(path)
                )
    ...
    async def find_best_path(
        self, token_in: str, token_out: str, amount_in: int, max_hops: int = 3
        ...
        try:
            self.logger.info(
                "Finding best path from %s to %s", token_in[:8], token_out[:8]
            )
            ...
            if not common_tokens:
                ...
                amount_out = await self.get_quote_from_quoter(
            ...
            direct_amount = await self.get_quote_from_quoter(
                amount_in, [token_in, token_out]
            )
            if direct_amount:
            ...
            if max_hops >= 2:
                for intermediate in common_tokens:
                    ...
                    try:
                        ...
                        self.logger.debug(
                            "Error checking 2-hop path %s: %s",
                            "->".join([t[:6] for t in path]),
                            str(e),
                        )
                        continue
            ...
            if best_path:
                ...
                gas_estimate = (
                    150000
                    if len(best_path) == 2
                    else await self.get_multi_hop_gas_estimate(best_path)
                )
    ...
    async def get_quote_with_impact(
        ...
        try:
            ...
            self.logger.debug(
                "Getting quote with impact for path of length %d", len(path)
            )
            ...
            if len(path) > 2:
                ...
                if not amount_out:
                    self.logger.warning(
                        "Failed to get quote for amount_in: %s, path length: %d",
                        amount_in,
                        len(path),
                    )
                    return None
                ...
                if not amount_out:
                    self.logger.warning(
                        "Failed to get quote for amount_in: %s, path: %s",
                        amount_in,
                        path,
                    )
                    return None
                ...
                if not baseline_out:
                    self.logger.warning(
                        "Failed to get baseline quote for amount_in: %s, path: %s",
                        small_amount,
                        path,
                    )
                    return None
            ...
            return {
                ...
                "fee_rate": float(self.fee)
                / 1000000,  # Convert to float to avoid decimal division
    ...
    async def get_token_price(self, token_address: str) -> float:
        ...
        try:
            ...
            if quote:
                ...
                quote_value = (
                    float(quote) if isinstance(quote, (int, float, str, Decimal)) else 0
                )
    ...
    async def get_total_liquidity(self) -> Decimal:
        ...
        try:
            ...
            event_abi = next(
                e
                for e in self.factory.abi
                if e["type"] == "event" and e["name"] == "PoolCreated"
            )
    ...
    async def build_swap_transaction(
        ...
        try:
            ...
            tx = {
                ...
                "data": self.router.encodeABI(fn_name="exactInput", args=[params]),
