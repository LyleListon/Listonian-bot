...
class DexManager:
    ...
    def __init__(self, web3_manager: Web3Client, dexes: Dict[str, DexInfo]):
        """
    ...
    @classmethod
    async def create(
        cls, web3_manager: Web3Client, config: Dict[str, Any]
    ) -> "DexManager":
        """
    ...
    async def discover_pools(
        self, dex_name: str, token_addresses: Optional[List[ChecksumAddress]] = None
    ) -> Set[ChecksumAddress]:
        """
        ...
        try:
            ...
            if not dex.factory_abi:
                logger.warning(
                    f"No factory ABI for {dex_name}, skipping pool discovery"
                )
                return set()

            # Create factory contract
            factory_contract = self.web3_manager.contract(
                address=dex.factory_address, abi=dex.factory_abi
            )

            # Check DEX type based on version and name
            if dex.name == "aerodrome":
                return await self.discover_pools_aerodrome(
                    factory_contract, dex, token_addresses
                )
            elif (
                dex.version == "v3" or dex.name == "swapbased"
            ):  # swapbased uses v3 style despite config
    ...
    async def get_price(
        ...
        try:
            ...
            pool_contract = self.web3_manager.contract(address=pool, abi=dex.pool_abi)
            ...
            logger.debug(
                f"Failed to get price for {token_in}/{token_out} on {dex_name}: {e}"
            )
            return 0

    @with_retry(max_attempts=3, base_delay=1.0)
    async def get_pool(
        self, dex_name: str, token_in: ChecksumAddress, token_out: ChecksumAddress
        ...
        try:
            ...
            logger.debug(
                f"Failed to get pool for {token_in}/{token_out} on {dex_name}: {e}"
            )
            return None

    @with_retry(max_attempts=3, base_delay=1.0)
    async def get_fee(self, dex_name: str, pool_address: ChecksumAddress) -> int:
        """Get pool fee in basis points."""
        try:
            dex = self.dexes[dex_name]
            pool_contract = self.web3_manager.contract(
                address=pool_address, abi=dex.pool_abi
            )
    ...
    @with_retry(max_attempts=3, base_delay=1.0)
    async def get_pool_liquidity(self, token_address: ChecksumAddress) -> int:
        """
        ...
        for dex in self.dexes.values():
            ...
            for pool in dex.pools:
                try:
                    ...
                    # Get token balance
                    balance = await pool_contract.functions.balanceOf(
                        token_address
                    ).call()
    ...
    async def calculate_price_impact(
        self, dex_name: str, pool_address: ChecksumAddress, amount_in: int
    ) -> float:
        """
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.debug(
                        f"Failed to calculate price impact for pool {pool_address}: {e}"
                    )
