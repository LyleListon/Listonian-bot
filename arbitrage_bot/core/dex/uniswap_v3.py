...
class UniswapV3DEX(BaseDEXV3):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.quoter = self.w3.eth.contract(
                address=self.config["quoter"], abi=self.quoter_abi
            )
    ...
    async def get_quote_with_impact(
        ...
        try:
            ...
            quote = await self.quoter.functions.quoteExactInputSingle(
                path[0], path[1], self.config["fee"], amount_in, 0  # sqrtPriceLimitX96
            ).call()

            # Calculate price impact
            price_impact = self._calculate_price_impact_v3(
                amount_in, quote.amountOut, slot0.sqrtPriceX96, liquidity
            )
    ...
    async def swap_exact_tokens_for_tokens(
        ...
        try:
            ...
            signed_tx = self.w3.eth.account.sign_transaction(
                tx, private_key=self.config.get("wallet", {}).get("private_key")
            )
            ...
            # Log transaction
            self._log_transaction(tx_hash.hex(), amount_in, amount_out_min, path)
    ...
    def _calculate_price_impact_v3(
        self, amount_in: int, amount_out: int, sqrt_price_x96: int, liquidity: int
    ) -> float:
