...
class DistributionManager:
    ...
    def __init__(
        ...
        self.min_balance = Decimal(str(self.config.get('min_balance', 0.1)))  # Minimum 0.1 ETH
        self.gas_buffer = Decimal(str(self.config.get('gas_buffer', 0.05)))  # 0.05 ETH buffer for gas
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if not self.web3_manager or not hasattr(self.web3_manager, 'get_balance'):
                raise ValueError("web3_manager must be provided and implement get_balance method")
            
            # Verify profit recipient
            if not self.profit_recipient:
                self.profit_recipient = self.web3_manager.wallet_address
                logger.info("Using wallet address as profit recipient: %s", self.profit_recipient)
