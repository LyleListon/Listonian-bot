...
import time
import uuid
from typing import Any, Callable, Dict, List, Optional, Set, TypeVar, Generic, Union, cast
            ...
            try:
                if asyncio.iscoroutinefunction(handler):
                    ...
                    await loop.run_in_executor(
                        None, cast(SyncHandler, handler), event
                    )
            except Exception as e:
                logger.error("Error calling event handler: %s", str(e))
    
    async def _run_async_handler(self, handler: AsyncHandler, event: Event[Any]) -> None:
