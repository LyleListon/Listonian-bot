...
from arbitrage_bot.core.events.dex_events import (
    DEXEventMonitor,
    SwapEvent,
    LiquidityEvent,
)
from arbitrage_bot.core.events.opportunity_tracker import (
    OpportunityTracker,
    ArbitrageOpportunity,
)
from arbitrage_bot.core.events.transaction_monitor import (
...
class EventSystem:
        ...
        async with self._lock:
            ...
            if self.web3_manager:
                ...
                if not self.transaction_monitor:
                    self.transaction_monitor = TransactionLifecycleMonitor(
                        event_emitter=self.event_emitter, web3_manager=self.web3_manager
                    )
            ...
            if not self.opportunity_tracker:
                ...
                self.opportunity_tracker = OpportunityTracker(
                    event_emitter=self.event_emitter, data_dir=opportunity_dir
                )
    ...
    async def emit(
        ...
        await self.event_emitter.emit(
            event_name, data, source=source, severity=severity
        )

    async def subscribe(self, event_name: str, callback: Any) -> int:
        """
        Subscribe to events.
    ...
    async def wait_for_event(
        self, event_name: str, timeout: Optional[float] = None, predicate=None
    ) -> Optional[Event[Any]]:
        """
    ...
    def get_recent_swap_events(
        ...
        return self.dex_monitor.get_recent_swap_events(
            token_addresses=token_addresses, dex_names=dex_names, limit=limit
        )
    ...
    def get_event_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            "subscriptions": {
                ...
                "by_event": {
                    name: len(subs) for name, subs in self._subscriptions.items()
                },
            }
        }
        ...
        if self.dex_monitor:
            stats["dex_monitor"] = {
                ...
                "liquidity_events_cache_size": len(
                    self.dex_monitor._liquidity_events_cache
                ),
            }
...
async def create_event_system(
    web3_manager=None, dex_manager=None, data_dir: Optional[str] = None
) -> EventSystem:
    """
    ...
    # Create event system
    system = EventSystem(
        web3_manager=web3_manager, dex_manager=dex_manager, data_dir=data_dir
    )
