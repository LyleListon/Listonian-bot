...
class TransactionStatus(str, Enum):
    ...
    CONFIRMING = "confirming"     # Transaction included in a block but waiting for confirmations
    ...
    BUNDLE_SIMULATED = "bundle_simulated"  # Bundle simulated successfully but not submitted
...
class TransactionRecord:
    ...
    submission_timestamp: float = field(default_factory=time.time)  # When tx was submitted
    ...
    def calculate_profit_accuracy(self) -> float:
        ...
        accuracy = 1.0 - min(1.0, abs(self.actual_profit - self.expected_profit) / abs(self.expected_profit))
...
class TransactionLifecycleMonitor:
    ...
    async def _register_event_handlers(self) -> None:
        ...
        await self.event_emitter.on('transaction:submitted', self._handle_transaction_submitted)
        
        # Bundle submission handler
        await self.event_emitter.on('flashbots:bundle_submitted', self._handle_bundle_submitted)
        
        # Bundle included handler
        await self.event_emitter.on('flashbots:bundle_included', self._handle_bundle_included)
        
        # Bundle simulation results
        await self.event_emitter.on('flashbots:bundle_simulated', self._handle_bundle_simulated)
    
    async def _unregister_event_handlers(self) -> None:
        """Unregister event handlers."""
        await self.event_emitter.off('transaction:submitted', self._handle_transaction_submitted)
        await self.event_emitter.off('flashbots:bundle_submitted', self._handle_bundle_submitted)
        await self.event_emitter.off('flashbots:bundle_included', self._handle_bundle_included)
        await self.event_emitter.off('flashbots:bundle_simulated', self._handle_bundle_simulated)
        ...
        try:
            ...
            for tx_hash in tx_hashes:
                if tx_hash in self._transactions:
                    ...
                    tx_record.bundle_transactions = [h for h in tx_hashes if h != tx_hash]
        ...
        try:
            ...
            for tx_hash in tx_hashes:
                if tx_hash in self._transactions:
                    ...
                    tx_result = next((r for r in simulation_results.get('results', []) 
                    ...
                    if success:
                        ...
                        tx_record.error = tx_result.get('error') or simulation_results.get('error')
        ...
        async with self._lock:
            ...
            if tx_hash in self._transactions:
                ...
                if status is not None:
                    record.status = TransactionStatus(status) if isinstance(status, str) else status
                ...
                record = TransactionRecord(
                    ...
                    status=TransactionStatus(status) if isinstance(status, str) else (status or TransactionStatus.PENDING),
                    bundle_id=bundle_id,
                ...
                if record.status in [TransactionStatus.PENDING, TransactionStatus.BUNDLE_PENDING]:
                    self._pending_transactions.add(tx_hash)
    ...
    async def update_transaction_status(
        ...
        async with self._lock:
            ...
            if record.status == TransactionStatus.CONFIRMED and old_status != TransactionStatus.CONFIRMED:
    ...
    def _trim_transactions(self) -> None:
        ...
        sorted_transactions = sorted(
            non_pending.items(),
            key=lambda x: x[1].submission_timestamp,
            reverse=True
        )
        ...
        keep_count = min(
            ...
            self.max_track_transactions - len(pending)
        )
        ...
        for i in range(0, len(pending_list), batch_size):
            ...
            for tx_hash in batch:
                try:
                    ...
                    receipt = await self.web3_manager.w3.eth.get_transaction_receipt(tx_hash)
    ...
    async def _check_pending_bundles(self) -> None:
        ...
        for bundle_id, tx_hashes in list(self._pending_bundles.items()):
            ...
            if target_block and current_block > target_block + 2:
                ...
                logger.info(f"Bundle {bundle_id} missed target block {target_block}, current block {current_block}")
    ...
    def get_transactions(
        ...
        for record in self._transactions.values():
            ...
            if min_submission_time and record.submission_timestamp < min_submission_time:
                continue
                
            if max_submission_time and record.submission_timestamp > max_submission_time:
                continue
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        success_rate = self._success_count / total_completed if total_completed > 0 else 0
        
        # Calculate average profit accuracy
        avg_profit_accuracy = self._profit_accuracy_sum / self._profit_accuracy_count if self._profit_accuracy_count > 0 else 0
