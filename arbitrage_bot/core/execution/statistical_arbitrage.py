...
class StatisticalArbitrage:
    ...
    async def analyze_price_patterns(
        self, token: str, prices: Dict[str, Dict[str, float]]
        ...
        try:
            ...
            analysis = await call_mcp_tool(
                ...
                {"token": token, "days": 1, "min_profit_threshold": self.min_profit},
            )
            ...
            for dex, pools in prices.items():
                for pool_type, price in pools.items():
                    price_points.append({"dex": dex, "pool": pool_type, "price": price})
            ...
            return {
                "opportunities": sorted(
                    opportunities, key=lambda x: x["profit_potential"], reverse=True
                ),
                "market_conditions": {
    ...
    async def detect_mean_reversion(
        ...
        try:
            ...
            analysis = await call_mcp_tool(
                "market-analysis", "analyze_opportunities", {"token": token, "days": 1}
            )
            ...
            if z_score > 2.0:  # Statistically significant deviation
                return {
                    ...
                    "z_score": z_score,
                    "expected_direction": (
                        "up" if current_price < mean_price else "down"
                    ),
