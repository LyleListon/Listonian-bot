...
class TriangularArbitrage:
    ...
    async def find_triangular_routes(
        self, start_token: str, depth: int = 3
    ) -> List[Dict[str, Any]]:
        ...
        async def dfs(
            current_token: str, path: List[Dict[str, Any]], current_depth: int
        ):
            ...
            for dex_name, dex in self.dex_interfaces.items():
                for next_token in self.base_tokens:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"No pool for {current_token}-{next_token} on {dex_name}: {e}"
                        )
    ...
    async def calculate_profit(
        self, route: List[Dict[str, Any]], amount_in: int
                ...
                if hop["dex"] == "aerodrome":
                    ...
                    try:
                        amounts_stable = await dex.get_amounts_out(
                            amounts[-1], [hop["token0"], hop["token1"]], [True]
                        )
                        amounts_volatile = await dex.get_amounts_out(
                            amounts[-1], [hop["token0"], hop["token1"]], [False]
                        )
                        # Use the better rate
                    ...
                    amounts_out = await dex.get_amounts_out(
                        amounts[-1], [hop["token0"], hop["token1"]]
                    )
                    amount_out = amounts_out[-1]
    ...
    async def find_opportunities(
        self, start_amount: int = 10**18  # 1 token
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for route in routes:
                profit_percent, amounts = await self.calculate_profit(
                    route, start_amount
                )
                        ...
                        opportunities.append(
                            {
                                ...
                                "amounts": amounts,
                                "market_conditions": analysis.get(
                                    "market_conditions", {}
                                ),
        ...
        try:
            ...
            for hop in opportunity["route"]:
                liquidity_check = await call_mcp_tool(
                    ...
                    {"token": hop["token0"], "metrics": ["liquidity"]},
                )

                if (
                    not liquidity_check
                    or liquidity_check.get("liquidity_score", 0) < 0.5
                ):
                    logger.warning(
                        f"Insufficient liquidity for {hop['token0']} on {hop['dex']}"
                    )
                    return False
            ...
            if (
                market_check.get("volatility", 1.0) > 0.1  # High volatility
                or market_check.get("volume_trend", 0) < 0  # Declining volume
                or market_check.get("liquidity_score", 0) < 0.5  # Low liquidity
            ):
