...
DEFAULT_CONTRACTS = {
    ...
    "mainnet": {"lending_pool": "0x87870Bca3F3fD6335C3F4ce8392D69350B4fA4E2"},
    # Aave V3 Lending Pool on Polygon
    "polygon": {"lending_pool": "0x794a61358D6845594F94dc1DB02A252b5b4814aD"},
    # Aave V3 Lending Pool on Arbitrum
    "arbitrum": {"lending_pool": "0x794a61358D6845594F94dc1DB02A252b5b4814aD"},
    # Aave V3 Lending Pool on Optimism
    "optimism": {"lending_pool": "0x794a61358D6845594F94dc1DB02A252b5b4814aD"},
    # Aave V3 Lending Pool on Base
    "base": {"lending_pool": "0xA238Dd80C259a72e81d7e4664a9801593F98d1c5"},
}
...
class AaveFlashLoanProvider:
    ...
    def __init__(self, web3_client: Web3Client, config: Dict[str, Any] = None):
        ...
        if not self.lending_pool_address:
            raise ValueError(
                f"No Aave Lending Pool address found for network: {self.network}"
            )
        ...
        logger.info(
            f"Created AaveFlashLoanProvider for network {self.network} "
        ...
        async with self._initialization_lock:
            ...
            try:
                ...
                self.lending_pool_contract = self.web3_client.get_contract(
                    address=self.lending_pool_address, abi=lending_pool_abi
                )
    ...
    async def execute_flash_loan(
        ...
        async with self._flash_loan_lock:
            ...
            logger.info(
                f"Executing Aave flash loan for {len(params.token_amounts)} tokens"
            )
