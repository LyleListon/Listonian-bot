...
class BalancerFlashLoanProvider:
    ...
    def __init__(
        ...
        self.vault_address = self.config.get(
            "vault_address", 
            DEFAULT_CONTRACTS.get(self.network, {}).get("vault")
        )
        
        if not self.vault_address:
            raise ValueError(f"No Balancer Vault address found for network: {self.network}")
        ...
        logger.info(f"Created BalancerFlashLoanProvider for network {self.network} "
    ...
    async def execute_flash_loan(
        ...
        async with self._flash_loan_lock:
            ...
            logger.info(f"Executing Balancer flash loan for {len(params.token_amounts)} tokens")
