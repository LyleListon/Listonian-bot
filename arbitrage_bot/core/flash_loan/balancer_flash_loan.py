    ...
    async def estimate_fees(
        ...
        try:
            ...
            for i, token in enumerate(tokens):
                fee = await self.vault_contract.functions.getFlashLoanFeePercentage().call()
...
async def create_balancer_flash_loan(
    w3: Web3,
    config: Dict
) -> BalancerFlashLoan:
    """
    ...
    return BalancerFlashLoan(
        w3=w3,
        vault_address=config['flash_loan']['balancer_vault'],
        vault_abi=vault_abi
    )