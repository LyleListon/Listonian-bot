...
class FlashbotsProvider:
    ...
    async def initialize(self) -> None:
        ...
        async with self._lock:
            ...
            try:
                ...
                logger.debug(
                    f"Created Flashbots auth header for {self._account.address}"
                )
                ...
                logger.error(
                    f"Failed to initialize Flashbots provider: {e}", exc_info=True
                )
                raise
    ...
    async def simulate_bundle(
        self, transactions: List[TxParams], block_number: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for tx in transactions:
                ...
                if isinstance(tx, dict) and "rawTransaction" not in tx:
                    ...
                    # Already signed
                    signed_txs.append(
                        tx.rawTransaction.hex() if hasattr(tx, "rawTransaction") else tx
                    )
            ...
            result = await self._make_request("eth_callBundle", params)
    ...
    async def _make_request(
        ...
        try:
            ...
            # Send request
            response = await self._web3.provider.make_request(method, params)
            ...
            if retries < self._max_retries:
                logger.warning(
                    f"Retrying Flashbots request ({retries+1}/{self._max_retries}): {e}"
                )
    ...
    async def send_bundle(
        self, transactions: List[TxParams], target_block_number: Optional[int] = None
    ) -> str:
        """
        ...
        try:
            ...
            params = [{"txs": signed_txs, "blockNumber": hex(target_block_number)}]

            # Send bundle request
            response = await self._make_request("eth_sendBundle", params)
    ...
    async def get_bundle_stats(self, bundle_hash: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            # Send request
            response = await self._make_request("flashbots_getBundleStats", params)
    ...
    async def wait_for_bundle(
        self, bundle_hash: str, timeout: Optional[float] = None
    ) -> Dict[str, Any]:
        """
        ...
        while current_block <= target_block:
            try:
                ...
                if asyncio.get_event_loop().time() > deadline:
                    raise TimeoutError(f"Timeout waiting for bundle {bundle_hash}")
    ...
    async def get_user_stats(self) -> Dict[str, Any]:
        ...
        try:
            # Send request
            response = await self._make_request("flashbots_getUserStats", [{}])
