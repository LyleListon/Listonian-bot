...
class GasOptimizer:
    ...
    async def _update_gas_prices(self):
        ...
        try:
            ...
            self._last_priority_fee = (
                await self.web3_manager.w3.eth.max_priority_fee_per_gas
            )
    ...
    async def estimate_gas(
        self, tx_params: Dict[str, Any], dex_name: Optional[str] = None
    ) -> int:
        """
        ...
        async with self.lock:
            return {
                "maxPriorityFeePerGas": Web3.to_wei(
                    self.strategy.max_priority_fee, "gwei"
                ),
    ...
    async def update_strategy(self, new_strategy: Dict[str, Any]):
        ...
        async with self.lock:
            self.strategy = GasStrategy(
                max_priority_fee=float(
                    new_strategy.get("max_priority_fee", self.strategy.max_priority_fee)
                ),
                ...
                buffer_percent=float(
                    new_strategy.get("buffer_percent", self.strategy.buffer_percent)
                ),
            )
...
async def create_gas_optimizer(
    dex_manager: DexManager, web3_manager: Any, config: Optional[Dict[str, Any]] = None
) -> GasOptimizer:
    """
