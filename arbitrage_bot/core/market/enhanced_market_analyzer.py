    ...
    async def initialize(self) -> None:
        async with self._lock:
            ...
            try:
                ...
                logger.error(
                    f"Failed to initialize enhanced market analyzer: {e}", exc_info=True
                )
                raise

    async def analyze_market_data(
        self, market_data: Dict[str, Any], prices: List[float], sources: List[str]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            liquidity_dict = {
                src: market_data.get("liquidity", {}).get(src, 0) for src in sources
            }

            price_validation = await self._price_validator.validate_prices(
                token_pair=token_pair, prices=price_dict, liquidity=liquidity_dict
            )
            ...
            await self._update_state(
                market_data=market_data, price_validation=price_validation
            )
    ...
    async def cleanup(self) -> None:
        async with self._lock:
            ...
            try:
                ...
                logger.error(
                    f"Error cleaning up enhanced market analyzer: {e}", exc_info=True
                )
                raise

    async def _update_state(
        self, market_data: Dict[str, Any], price_validation: Dict[str, Any]
    ) -> None:
