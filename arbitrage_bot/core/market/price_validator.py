...
class PriceValidator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self._suspicious_change_threshold = config.get(
            ...
            try:
                ...
                for source, price in prices.items():
                    ...
                    if deviation > self._max_price_deviation:
                        return (
                            False,
                            f"Price deviation too high on {source}: {deviation:.2%}",
                            0.0,
                        )
                ...
                if total_liquidity < self._min_liquidity_usd:
                    return (
                        False,
                        f"Insufficient liquidity: ${total_liquidity:,.2f}",
                        0.0,
                    )

                # Check for manipulation
                is_suspicious, reason = await self._check_manipulation(
                    token_pair[0], mean_price
                )
                if is_suspicious:
                ...
                confidence = self._calculate_confidence(
                    prices=prices, liquidity=liquidity, mean_price=mean_price
                )

                # Update price history
                await self._update_price_history(token=token_pair[0], price=mean_price)
            ...
            if len(recent_changes) >= 3:
                oscillations = sum(
                    ...
                    if (
                        ...
                        and recent_changes[i + 1] < recent_changes[i + 2]
                    )
                    or (
                        recent_changes[i] < recent_changes[i + 1]
                        and recent_changes[i + 1] > recent_changes[i + 2]
    ...
    def _calculate_confidence(
        self, prices: Dict[str, float], liquidity: Dict[str, float], mean_price: float
    ) -> float:
        ...
        max_deviation = max(
            abs(price - mean_price) / mean_price for price in prices.values()
        )
    ...
    async def _update_price_history(self, token: str, price: float) -> None:
