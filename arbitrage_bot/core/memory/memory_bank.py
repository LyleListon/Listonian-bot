...
class MemoryBank:
    ...
    async def add_trade(self, trade: Dict[str, Any]) -> None:
        ...
        async with self._lock:
            ...
            try:
                file_path = (
                    self._trades_dir / f"trade_{int(timestamp.timestamp())}.json"
                )
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        if (
            datetime.now() - self._metrics_timestamp
        ).total_seconds() > self._metrics_ttl:
            return {}
    ...
    async def update_state(self, state: Dict[str, Any]) -> None:
        ...
        async with self._lock:
            ...
            try:
                ...
                await self._file_manager.write_json(
                    file_path,
                    {
    ...
    async def _update_stats(self, trade: Dict[str, Any]) -> None:
        """
