...
class PerformanceOptimizer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.max_tasks = config.get("process", {}).get("max_tasks", 100)        # concurrent tasks
    ...
    def _init_resources(self):
        ...
        try:
            ...
            # Set process priority (Windows equivalent of nice)
    ...
    async def get_metrics(self) -> PerformanceMetrics:
        ...
        try:
            ...
            if self.last_network_stats:
                ...
                rx_mb = (net_stats.bytes_recv - self.last_network_stats[1]) / (1024 * 1024 * time_diff)
                tx_mb = (net_stats.bytes_sent - self.last_network_stats[2]) / (1024 * 1024 * time_diff)
            else:
                rx_mb = tx_mb = 0
                
            self.last_network_stats = (time.time(), net_stats.bytes_recv, net_stats.bytes_sent)
        ...
        while True:
            try:
                ...
                if metrics.cpu_usage > self.max_cpu * 0.9:  # 90% threshold
                    ...
                    while len(self._active_tasks) > self.max_tasks * 0.8:  # 80% threshold
    ...
    def get_optimization_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_metrics = {
                ...
                'network_rx': sum(m.network_rx for m in self.metrics_history) / len(self.metrics_history),
                'network_tx': sum(m.network_tx for m in self.metrics_history) / len(self.metrics_history),
                'api_latency': sum(m.api_latency for m in self.metrics_history) / len(self.metrics_history)
            }
