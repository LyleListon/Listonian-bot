...
class WorkStealingExecutor:
    ...
    def __init__(
        ...
        # Worker locks
        self._locks: List[asyncio.Lock] = [asyncio.Lock() for _ in range(self.num_workers)]
        ...
        logger.debug(f"WorkStealingExecutor initialized with {self.num_workers} workers")
    ...
    async def _steal_task(self, worker_id: int) -> Optional[Task]:
        ...
        try:
            ...
            if not victim_queue.empty():
                ...
                logger.debug(f"Worker {worker_id} stole task {task.id} from worker {victim_id}")
                return task
        finally:
            # Release lock
    ...
    async def _try_lock(self, worker_id: int) -> bool:
        ...
        """
        try:
            return await asyncio.wait_for(self._locks[worker_id].acquire(), timeout=0.01)
        except asyncio.TimeoutError:
            return False
        ...
        while self._running:
            try:
                ...
                if cpu_percent > self.throttle_threshold:
                    ...
                    excess = (cpu_percent - self.throttle_threshold) / (self.max_cpu_percent - self.throttle_threshold)
                    self._throttle_factor = max(0.1, 1.0 - excess)
                    
                    if not self._throttling:
                        logger.debug(f"CPU usage {cpu_percent:.1f}% exceeds throttle threshold {self.throttle_threshold:.1f}%, throttling execution (factor: {self._throttle_factor:.2f})")
