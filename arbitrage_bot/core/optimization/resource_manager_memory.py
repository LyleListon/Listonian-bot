        ...
        while True:
            try:
                ...
                with self._lock:
                    ...
                    now = time.time()
                    expired = [i for i, (_, timestamp) in enumerate(self._available) if now - timestamp > self.ttl]
                    ...
                    if expired:
                        logger.debug(f"Removed {len(expired)} expired objects from pool")
        ...
        while self._running:
            try:
                ...
                if memory_percent > self.gc_threshold:
                    logger.debug(f"Memory usage {memory_percent:.1f}% exceeds GC threshold {self.gc_threshold:.1f}%, triggering garbage collection")
                    gc.collect()
                    self._gc_count += 1
