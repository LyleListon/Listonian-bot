...
class OptimizedWebSocketClient:
    ...
    async def send(self, data: Dict[str, Any], priority: MessagePriority = MessagePriority.NORMAL) -> bool:
        """
        ...
        async with self._queue_lock:
            if len(self._queue) >= self.max_queue_size:
                ...
                if lowest_priority.priority > priority:
                    logger.warning(f"Queue full, dropping message with priority {lowest_priority.priority}")
                    self._queue.remove(lowest_priority)
                else:
                    logger.warning(f"Queue full, dropping new message with priority {priority}")
                    return False
    ...
    async def register_handler(self, message_type: str, handler: Callable[[Dict[str, Any]], Awaitable[None]]) -> None:
        """
    ...
    async def _reconnect(self) -> None:
        ...
        while not self._connected:
            logger.info(f"Reconnecting in {self._current_reconnect_interval:.1f} seconds")
    ...
    async def _send_batch(self) -> None:
        ...
        # Serialize and send
        try:
            serialized_data = self._serialize(batch_data, any(msg.compress for msg in batch))
    ...
    def _serialize(self, data: Dict[str, Any], compress: bool = False) -> Union[str, bytes]:
        """
        ...
        if self.format == MessageFormat.JSON:
            ...
            return serialized
        elif self.format == MessageFormat.MSGPACK or self.format == MessageFormat.MSGPACK_COMPRESSED:
...
class WebSocketConnectionPool:
    ...
    def __init__(
        ...
        logger.debug(f"WebSocketConnectionPool initialized with max_connections={max_connections}")
    ...
    async def send(self, url: str, data: Dict[str, Any], priority: MessagePriority = MessagePriority.NORMAL) -> bool:
        """
    ...
    async def register_handler(self, url: str, message_type: str, handler: Callable[[Dict[str, Any]], Awaitable[None]]) -> None:
        """
        Register a message handler.
