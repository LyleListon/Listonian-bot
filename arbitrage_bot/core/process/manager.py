...
class ProcessManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.max_memory_per_instance = config.get("process", {}).get("max_memory_mb", 1024)  # MB
        self.max_cpu_per_instance = config.get("process", {}).get("max_cpu_percent", 50)  # %
        self.heartbeat_timeout = config.get("process", {}).get("heartbeat_timeout", 30)  # seconds
                ...
                for instance_id, state in list(self.processes.items()):
                    try:
                        ...
                        if memory_mb > self.max_memory_per_instance:
                            logger.warning(f"Instance {instance_id} exceeded memory limit")
