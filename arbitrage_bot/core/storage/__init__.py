...
class StorageManager(Generic[T]):
        ...
        if metadata_path.exists():
            try:
                with open(metadata_path, "r") as f:
                    data = json.load(f)
                    self._metadata = {k: StorageMetadata(**v) for k, v in data.items()}
    ...
    def store(
        ...
        metadata = StorageMetadata(
            ...
            backup_path=(
                str(self.base_path / "backups" / (key + "_" + str(now) + ".json"))
                if create_backup and key in self._metadata
                else None
            ),
        )
    ...
    def restore_backup(
        self, key: str, timestamp: Optional[float] = None
    ) -> StorageMetadata:
    ...
    def retrieve_recent(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        for key in sorted_keys:
            ...
            if data:
                items.append(
                    {"key": key, "data": data, "metadata": asdict(self._metadata[key])}
                )
