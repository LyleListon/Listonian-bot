...
from unittest.mock import Mock, AsyncMock, MagicMock # Removed patch
# from typing import Dict, Any # Unused

from ..execution.arbitrage_executor import ArbitrageExecutor # Removed create_arbitrage_executor
# from ..web3.web3_manager import Web3Manager # Unused
# from ..dex import DexManager # Unused
# from ..gas.gas_optimizer import GasOptimizer # Unused
...
async def web3_manager():
    ...
    manager.use_mcp_tool = AsyncMock(return_value={"ethereum": {"usd": 2000.0}})
    return manager
...
async def market_analyzer():
    ...
    analyzer = AsyncMock()
    analyzer.get_market_condition = AsyncMock(
        return_value={
            "price": 2000.0,
            "volume": 1000000.0,
    ...
    analyzer.market_conditions = {"WETH": MagicMock(price=Decimal("2000.0"))}
    return analyzer
...
async def dex_manager():
    ...
    dex1.get_quote_with_impact = AsyncMock(
        return_value={
    ...
    dex2.get_quote_with_impact = AsyncMock(
        return_value={
    ...
    manager.get_dex = Mock(
        side_effect=lambda name: dex1 if name == "BaseSwap" else dex2
    )
...
async def test_price_conversion_with_caching(executor):
    ...
    assert (
        executor.market_analyzer.get_market_condition.call_count == 1
    )  # Should use cache
