...
from unittest.mock import AsyncMock # Removed Mock, patch, MagicMock
# from typing import Dict, Any # Unused

from ..analysis.market_analyzer import MarketAnalyzer # Keep MarketAnalyzer import
from ..models.market_models import ( # Import models from correct location
    MarketCondition,
    MarketTrend,
    PricePoint,
)
...
async def web3_manager():
    ...
    manager.use_mcp_tool = AsyncMock(
        side_effect=lambda server, tool, args: {
            "crypto-price": {
                "ethereum": {"usd": 2000.0, "volume_24h": 1000000.0},
                "usd-coin": {"usd": 1.0, "volume_24h": 500000.0},
            },
            "market-analysis": {
                "metrics": {"volatility": 0.02, "volume": 1000000.0, "trend": "up"}
            },
        }.get(server)
    )
    return manager
...
def config():
    ...
    return {
        "tokens": {
            "WETH": {"address": "0x123...", "decimals": 18},
            "USDC": {"address": "0x456...", "decimals": 6},
        },
        "dexes": {
            "baseswap": {"factory": "0x789...", "router": "0xabc...", "fee": 3000},
            "pancakeswap": {"factory": "0xdef...", "router": "0xghi...", "fee": 3000},
        },
...
async def test_error_handling_and_fallbacks(market_analyzer):
    ...
    market_analyzer.web3_manager.use_mcp_tool = AsyncMock(
        side_effect=lambda server, tool, args: {
            "crypto-price": {"ethereum": {"usd": 2000.0, "volume_24h": 1000000.0}}
        }.get(server)
    )
...
async def test_report_generation(market_analyzer):
    ...
    # Add some test data
    market_analyzer.transactions = [
        {"timestamp": time.time() - 3600, "profit_usd": 1.5, "status": "success"}
    ]
    market_analyzer.alerts = [
        {"timestamp": time.time(), "level": "warning", "message": "Test alert"}
    ]
