...
class UnifiedBalanceManager:
    ...
    async def approve_token(
        self, token_address: str, spender_address: str, amount: Optional[int] = None
    ) -> bool:
        """
        ...
        try:
            ...
            current_allowance = await self.get_allowance(
                token_address, spender_address, force_refresh=True
            )
    ...
    async def check_and_approve(
        ...
        try:
            ...
            return await self.approve_token(
                token_address, spender_address, max(amount, self.min_approval_amount)
            )
