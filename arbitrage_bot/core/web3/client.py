        ...
        async with self._initialization_lock:
            ...
            try:
                ...
                for attempt in range(max_retries):
                    try:
                        logger.debug(f"Testing connection (attempt {attempt + 1}/{max_retries})")
                        ...
                        logger.info(f"Connected to {self.chain} at block {block_number}")
                ...
                logger.error(f"Failed to initialize Web3 client: {str(e)}", exc_info=True)
                raise
