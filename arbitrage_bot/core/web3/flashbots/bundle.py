...
class FlashbotsBundle:
    ...
    async def _estimate_priority_fee(self) -> int:
        ...
        avg_priority_fee = total_priority_fee / (
            blocks_to_check * len(block["transactions"])
        )
        ...
        async with self._simulation_lock:
            try:
                ...
                return BundleSimulation(
                    ...
                    mev_value=result[4],
                    state_changes=[
                        {"address": change[0], "value": change[1]}
        ...
        async with self._optimization_lock:
            ...
            for gas_price in variations:
                ...
                results = await self.simulate_bundle(
                    modified_txs, target_block, state_overrides=initial_state
                )

                if results.success:
                    profit = results.mev_value - (
                        results.gas_used * results.effective_gas_price
                    )
                    ...
                    for change in results.state_changes:
                        if change["address"] in initial_balances:
                            slippage = (
                                abs(
                                    change["value"]
                                    - initial_balances[change["address"]]
                                )
                                / initial_balances[change["address"]]
                            )
                            if slippage > self.max_slippage:
                                logger.warning(
                                    f"High slippage detected: {slippage * 100}%"
                                )
                                continue
    ...
    async def submit_bundle(
        ...
        try:
            ...
            optimized_txs, sim_results = await self.optimize_bundle(
                transactions, target_block, initial_state
            )

            # Validate profitability
            profit = sim_results.mev_value - (
                sim_results.gas_used * sim_results.effective_gas_price
            )
