...
class BundleSimulator:
    ...
    async def simulate(
        ...
        result = BundleSimulationResult(success=False, error="Simulation not attempted")

        try:
            logger.info(
                f"Simulating bundle with {len(bundle.signed_transactions)} transaction(s)"
            )
    ...
    async def calculate_profit(
        self, simulation_result: BundleSimulationResult
    ) -> float:
        """
    ...
    async def verify_profitability(
        ...
        if is_profitable:
            ...
            logger.warning(
                f"Bundle is not profitable: {profit} ETH < {min_profit_threshold} ETH"
            )
