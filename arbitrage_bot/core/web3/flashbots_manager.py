...
class FlashbotsManager:
    ...
    def __init__(
        ...
        self._min_profit_threshold = (
            0  # Min profit in wei to consider bundle profitable
        )
        ...
        logger.info(
            "Initialized Flashbots manager with profit threshold: %d wei",
            self._min_profit_threshold,
        )
    ...
    async def setup_flashbots_provider(self):
        ...
        try:
            ...
            self.web3_manager.w3.flashbots = FlashbotsProvider(
                self.web3_manager.w3, self.auth_signer, self.relay_url
            )
    ...
    async def _setup_balance_validator(self):
        ...
        try:
            ...
            logger.warning(
                "Balance validator not available, bundle validation will be limited"
            )
        except Exception as e:
    ...
    async def close(self):
        """Clean up resources."""
        try:
            if hasattr(self.web3_manager.w3, "flashbots") and hasattr(
                self.web3_manager.w3.flashbots, "close"
            ):
    ...
    async def simulate_bundle(self, bundle_id, block_tag="latest"):
        ...
        try:
            ...
            simulation = await self.web3_manager.w3.flashbots.simulate(
                bundle["txs"], block_tag=block_tag
            )
    ...
    async def submit_bundle(self, bundle_id, target_block=None) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.balance_validator:
                ...
                if bundle_simulation:
                    ...
                    # Validate bundle
                    validation_result = (
                        await self.balance_validator.validate_bundle_balance(
                            bundle_id, list(token_addresses)
                        )
                    )
                    logger.info(
                        f"Bundle {bundle_id} validation: {validation_result['success']}"
                    )
            ...
            result = await self.web3_manager.w3.flashbots.send_bundle(
                bundle["txs"], target_block_number=target
            )
    ...
    async def calculate_bundle_profit(
        ...
        try:
            ...
            if not simulation:
                raise ValueError(
                    "No simulation results for bundle {}".format(bundle_id)
                )
            ...
            if isinstance(coinbase_payment, str):
                coinbase_payment = (
                    int(coinbase_payment, 16)
                    if coinbase_payment.startswith("0x")
                    else int(coinbase_payment)
                )
            ...
            if isinstance(gas_used, str):
                gas_used = (
                    int(gas_used, 16) if gas_used.startswith("0x") else int(gas_used)
                )

            gas_price = simulation.get("gasPrice", 0)
            if isinstance(gas_price, str):
                gas_price = (
                    int(gas_price, 16) if gas_price.startswith("0x") else int(gas_price)
                )
            ...
            if token_transfers:
                for token, amount in token_transfers.items():
                    try:
                        ...
                        profit_result["errors"].append(
                            f"Failed to calculate value for token {token}: {str(e)}"
                        )
            ...
            if "balances" in simulation and account_to_check:
                ...
                if account_to_check in simulation["balances"]:
                    ...
                    if isinstance(before, str):
                        before = (
                            int(before, 16) if before.startswith("0x") else int(before)
                        )
            ...
            logger.error(
                "Failed to calculate bundle profit (bundle_id=%s): %s",
                bundle_id,
                str(e),
            )
            raise

    async def optimize_bundle_gas(self, bundle_id, max_priority_fee):
        ...
        try:
            ...
            optimal_settings = {
                ...
                "maxPriorityFeePerGas": min(
                    max_priority_fee, base_fee
                ),  # Cap priority fee
                "gasLimit": block.get("gasLimit", 0) // 2,  # Use half block gas limit
            }

            logger.info(
                "Optimized gas settings for bundle %s: %s", bundle_id, optimal_settings
            )
                ...
                if (
                    ...
                    if len(topics) >= 3:
                        ...
                        if account and (from_addr == account or to_addr == account):
                            ...
                            transfers[token_address] = transfers.get(
                                token_address, 0
                            ) + (amount if to_addr == account else -amount)
    ...
    async def validate_and_submit_bundle(
        ...
        try:
            ...
            if validation["success"] or (
                min_profit is None and expected_net_change is None
            ):
                # Get target block
                bundle = self._pending_bundles.get(bundle_id)
                if not bundle:
