        ...
        for endpoint in self.endpoints:
            if endpoint.url == endpoint_url:
                ...
                if endpoint.consecutive_errors >= 3:
                    ...
                    logger.warning(f"Temporarily disabled RPC endpoint {endpoint_url} due to consecutive errors")
                
                # Special handling for rate limit errors
                if isinstance(error, Exception) and "429" in str(error):
                    logger.warning(f"Rate limit hit for {endpoint_url}, backing off exponentially")
                break
