...
# from eth_typing import ChecksumAddress # Unused
...
class WalletManager:
    ...
    def __init__(
        self, web3_manager: Web3Manager, dex_manager: DexManager, config: Dict[str, Any]
    ):
        """
        ...
        self.min_eth_balance = Web3.to_wei(
            wallet_config.get("min_eth_balance", 0.1), "ether"
        )
        self.max_eth_balance = Web3.to_wei(
            wallet_config.get("max_eth_balance", 1.0), "ether"
        )
        ...
        self.profit_withdrawal_threshold = Web3.to_wei(
            wallet_config.get("profit_withdrawal_threshold", 1.0), "ether"
        )
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.wallet_address = Web3.to_checksum_address(
                Web3().eth.account.from_key(self.config["web3"]["wallet_key"]).address
            )
            balance = await self.web3_manager.w3.eth.get_balance(self.wallet_address)
            ...
            logger.error(
                f"Failed to initialize wallet manager: {str(e)}", exc_info=True
            )
            return False
    ...
    async def check_and_maintain_balances(self):
        ...
        async with self._balance_lock:
            try:
                ...
                for token, target_ratio in self.target_token_ratios.items():
                    ...
                    if abs(current_ratio - target_ratio) > self.rebalance_threshold:
                        ...
                        await self._rebalance_token(
                            token, current_value_eth, target_ratio * total_value_eth
                        )
                ...
                logger.error(f"Balance maintenance error: {str(e)}", exc_info=True)
    ...
    async def _replenish_eth(self):
        ...
        try:
            eth_needed = (
                self.min_eth_balance
                - await self.web3_manager.w3.eth.get_balance(self.wallet_address)
            )
            logger.info(
            ...
            if best_token:
                ...
                await self._execute_swap(
                    best_token, self.config["tokens"]["WETH"]["address"], amount_in
                )
            else:
                logger.warning("No suitable token found for ETH replenishment")

        except Exception as e:
            logger.error(f"ETH replenishment error: {str(e)}", exc_info=True)
    ...
    async def _convert_excess_eth(self):
        ...
        try:
            ...
            logger.error(f"Excess ETH conversion error: {str(e)}", exc_info=True)

    @log_async_operation("token_rebalancing")
    async def _rebalance_token(
        self, token: str, current_value_eth: int, target_value_eth: int
    ):
        ...
        try:
            async with self._swap_lock:
                ...
                if current_value_eth < target_value_eth:
                    ...
                    await self._execute_swap(
                        self.config["tokens"]["WETH"]["address"], token, eth_needed
                    )
                else:
                    # Need to sell some
                    ...
                    await self._execute_swap(
                        token, self.config["tokens"]["WETH"]["address"], tokens_to_sell
                    )

        except Exception as e:
            logger.error(f"Token rebalancing error: {str(e)}", exc_info=True)
        finally:
            logger.debug("Released swap lock after token rebalancing")

    @log_async_operation("swap_execution")
    async def _execute_swap(self, token_in: str, token_out: str, amount_in: int):
        ...
        try:
            ...
            for dex_name in self.dex_manager.dexes:
                ...
                if quote and quote > best_output:
                    best_dex = dex
                    best_output = quote
                    logger.debug(f"Found better quote on {dex_name}: {best_output}")
            ...
            logger.error(f"Swap execution error: {str(e)}", exc_info=True)
    ...
    async def _check_profit_withdrawal(self):
        ...
        try:
            ...
            if eth_balance > self.profit_withdrawal_threshold:
                ...
                if profit_amount > 0:
                    ...
                    withdrawal_address = self.config["wallet"].get(
                        "profit_withdrawal_address", self.wallet_address
                    )
                    ...
                    signed_tx = self.web3_manager.w3.eth.account.sign_transaction(
                        tx, self.config["web3"]["wallet_key"]
                    )
            ...
            logger.error(f"Profit withdrawal error: {str(e)}", exc_info=True)


@log_async_operation("wallet_manager_creation")
async def create_wallet_manager(
    web3_manager: Web3Manager, dex_manager: DexManager, config: Dict[str, Any]
) -> WalletManager:
    """
