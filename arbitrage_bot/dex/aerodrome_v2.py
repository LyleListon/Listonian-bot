...
class AerodromeV2(BaseDEX):
    ...
    async def get_pool_address(
        ...
        try:
            ...
            # Get pool address
            pool_address = self.factory.functions.getPair(
                token0,
                token1,
                stable
            ).call()
            ...
            if pool_address == "0x0000000000000000000000000000000000000000":
                raise ValueError(f"No {'stable' if stable else 'volatile'} pool found for {token_a}/{token_b}")
        ...
        try:
            ...
            if metadata[4]:  # stable pool
                ...
                k_after = ((reserve_0 + amount_in) ** 3) * (reserve_1 - amount_out) + \
                         ((reserve_1 - amount_out) ** 3) * (reserve_0 + amount_in)
    ...
    async def get_pool_fee(self, pool_address: str) -> Decimal:
        ...
        try:
            ...
            return (Decimal(str(fee_bps)) / Decimal("1000000")).quantize(Decimal("0.000001"))  # Convert from basis points to decimal (500 -> 0.0005)
    ...
    async def get_pool_info(self, pool_address: str) -> Dict[str, Any]:
        ...
        try:
            ...
            info = {
                ...
                "stable": metadata[4],
                "fee": Decimal("0.0001") if metadata[4] else Decimal("0.0005")  # 0.01% for stable, 0.05% for volatile
            }
