...
class AerodromeV3(BaseDEX):
    ...
    def __init__(self, web3: Web3, config: Dict[str, Any]):
        ...
        logger.debug(
            f"Aerodrome V3 Config - Factory: {factory_address}, Router: {router_address}, Quoter: {quoter_address}"
        )
        ...
        self.quoter = self.w3.eth.contract(address=quoter_address, abi=self.quoter_abi)
    ...
    async def get_pool_address(self, token_a: str, token_b: str, fee: int) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Calling factory.getPool with token0={token0}, token1={token1}, fee={fee}"
            )
            # Get pool address
            ...
            if pool_address == "0x0000000000000000000000000000000000000000":
                raise ValueError(
                    f"No pool found for {token_a}/{token_b} with fee {fee}"
                )
    ...
    async def get_amounts_out(
        self, amount_in: Decimal, path: List[str], fee: int = 3000
    ) -> List[Decimal]:
        ...
        try:
            ...
            # Use quoter contract
            quote = self.quoter.functions.quoteExactInput(path, amount_in_wei).call()
    ...
    async def get_pool_fee(self, pool_address: str) -> Decimal:
        ...
        try:
            ...
            return (Decimal(str(fee_bps)) / Decimal("1000000")).quantize(
                Decimal("0.000001")
            )  # Convert from basis points to decimal (e.g., 100 -> 0.0001)
    ...
    async def get_pool_info(self, pool_address: str) -> Dict[str, Any]:
        ...
        try:
            ...
            info = {
                ...
                "price": price,
                "tick": slot0[1],
                "fee": (Decimal(str(fee)) / Decimal("1000000")).quantize(
                    Decimal("0.000001")
                ),  # Convert from basis points to decimal (e.g., 100 -> 0.0001)
            }
    ...
    async def estimate_gas(
        ...
        try:
            ...
            return self.router.functions.exactInput(
                (path, amount_in_wei, amount_out_min_wei, to)
            ).estimate_gas()
            ...
            return self.router.functions.exactInput(
            ...
            ).build_transaction(
                {
                    ...
                    "gas": await self.estimate_gas(
                        amount_in, amount_out_min, path, to, fee
                    ),
