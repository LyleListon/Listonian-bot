...
async def setup_gas_optimizer(
    ...
    try:
        ...
        gas_optimizer = await GasOptimizer.create_gas_optimizer(
            web3_manager=web3_manager, config=config
        )
...
async def use_enhanced_multi_hop(dex_manager: DexManager) -> bool:
    """
    ...
    try:
            ...
            if dex.__class__.__name__.endswith("V3") or "V3" in dex.__class__.__name__:
                try:
                    ...
                    if not hasattr(dex, "supported_fees"):
                        dex.supported_fees = dex.config.get(
                            "supported_fees", [500, 3000, 10000]
                        )
                        dex.fee_tier_names = {
                            100: "0.01%",
                            500: "0.05%",
        ...
        logger.info(
            f"Enhanced {updated_count} DEX instances with improved multi-hop support"
        )
    ...
    try:
        ...
        if path_finder is None:
            ...
            path_finder = await create_path_finder(
                web3_manager=web3_manager, dex_manager=dex_manager, config=config
            )
    ...
    try:
        ...
        if test_token_pairs is None:
            ...
            usdc_address = (
                config.get("tokens", {})
                .get("USDC", {})
                .get("address", "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913")
            )
            usdt_address = (
                config.get("tokens", {})
                .get("USDT", {})
                .get("address", "0x4D15a3a2286D883AF0AA1B3f21367843FAc63E07")
            )
            dai_address = (
                config.get("tokens", {})
                .get("DAI", {})
                .get("address", "0x50c5725949A6F0c72E6C4a641F24049A917DB0Cb")
            )
        ...
        for pair in test_token_pairs:
            ...
            logger.info(
                f"Test {token_in[-6:]} → {token_out[-6:]} completed in {execution_time:.3f}s - Paths found: {len(paths)}"
            )
        ...
        multi_hop_count = sum(
            1
            for r in results
            if r.get("best_path") and len(r.get("best_path", {}).get("tokens", [])) > 2
        )
        ...
        logger.info(
            f"Optimization test completed: {success_count}/{len(results)} successful"
        )
        return summary
...
async def main():
    ...
    parser.add_argument(
        "--test", action="store_true", help="Run a quick test after integration"
    )
    parser.add_argument(
        "--count", type=int, default=5, help="Number of test pairs to run"
    )
