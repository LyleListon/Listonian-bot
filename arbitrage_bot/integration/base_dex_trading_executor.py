...
if USE_MOCK_TRADING:
    logger.warning("!!! USING MOCK TRADING FOR TESTING PURPOSES ONLY !!! Set USE_MOCK_TRADING=false to use real trading")
...
class BaseDexTradingExecutor:
    ...
    def __init__(
        ...
        if USE_MOCK_TRADING:
            logger.warning("!!! BaseDexTradingExecutor USING MOCK TRADING FOR TESTING PURPOSES ONLY !!!")
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            if stats["trades_executed"] > 0:
                stats["avg_profit_usd"] = stats["total_profit_usd"] / stats["trades_executed"]
            else:
        ...
        while self._running:
            try:
                ...
                profitable_opportunities = [
                    ...
                    if hasattr(opp, 'is_profitable') and opp.is_profitable and Decimal(str(opp.net_profit_usd)) >= self.min_profit_usd
                ]
                    ...
                    try:
                        ...
                        if result["success"]:
                            async with self._lock:
                                ...
                                self._stats["total_profit_usd"] += float(result["profit_usd"])
                                self._stats["last_trade_time"] = datetime.now()
                            
                            logger.info(f"Successfully executed trade with profit: ${result['profit_usd']:.2f}")
                        else:
                            async with self._lock:
                                self._stats["failed_trades"] += 1
                            
                            logger.warning(f"Failed to execute trade: {result['error']}")
    ...
    async def _execute_trade(self, opportunity: ArbitrageOpportunity) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Executing trade for opportunity: {getattr(opportunity, 'id', 'unknown')}")
            ...
            actual_profit_usd = Decimal(str(actual_profit_wei)) * Decimal(str(output_price_usd)) / Decimal('1e18')
    ...
    async def _validate_opportunity(self, opportunity: ArbitrageOpportunity) -> bool:
        ...
        try:
            ...
            if net_profit_usd < self.min_profit_usd:
                logger.warning(f"Profit ({net_profit_usd}) below minimum threshold ({self.min_profit_usd})")
                return False
    ...
    async def _create_transactions(self, opportunity: ArbitrageOpportunity, gas_price: int) -> List[Dict[str, Any]]:
        ...
        try:
            if USE_MOCK_TRADING:
                ...
                # =====================================================================
                logger.warning("!!! MOCK TRANSACTION CREATION - FOR TESTING PURPOSES ONLY !!!")
                ...
                # =====================================================================
                logger.info(f"Creating real transactions for arbitrage opportunity: {getattr(opportunity, 'id', 'unknown')}")
                ...
                logger.info(f"Created {len(transactions)} transactions for arbitrage opportunity")
                return transactions
        except Exception as e:
    ...
    async def _simulate_bundle(self, bundle: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if USE_MOCK_TRADING:
                ...
                # !!! MOCK DATA - FOR TESTING PURPOSES ONLY !!!
                # =====================================================================
                logger.warning("!!! MOCK BUNDLE SIMULATION - FOR TESTING PURPOSES ONLY !!!")
                ...
                if not await self.flashbots_provider.is_initialized():
                    try:
                        ...
                        logger.error(f"Failed to initialize Flashbots provider: {str(e)}")
                        return {"success": False, "error": f"Failed to initialize Flashbots provider: {str(e)}"}
                ...
                try:
                    ...
                    return {"success": False, "error": f"Failed to get block number: {str(e)}"}
                ...
                try:
                    ...
                    if simulation_result and "result" in simulation_result:
                        ...
                        return {"success": True, "profit_wei": result.get("value", 0), "gas_used": result.get("gasUsed", 0)}
                    else:
                        return {"success": False, "error": f"Invalid simulation result: {simulation_result}"}
                
                except Exception as e:
                    logger.error(f"Failed to simulate bundle: {str(e)}")
                    return {"success": False, "error": f"Failed to simulate bundle: {str(e)}"}
        except Exception as e:
    ...
    async def _submit_bundle(self, bundle: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if USE_MOCK_TRADING:
                ...
                # !!! MOCK DATA - FOR TESTING PURPOSES ONLY !!!
                # =====================================================================
                logger.warning("!!! MOCK BUNDLE SUBMISSION - FOR TESTING PURPOSES ONLY !!!")
                ...
                try:
                    ...
                    return {"success": False, "error": f"Failed to submit bundle: {str(e)}"}
        except Exception as e:
    ...
    async def _wait_for_inclusion(self, bundle_hash: str) -> Dict[str, Any]:
        ...
        try:
            if USE_MOCK_TRADING:
                ...
                # !!! MOCK DATA - FOR TESTING PURPOSES ONLY !!!
                # =====================================================================
                logger.warning("!!! MOCK BUNDLE INCLUSION - FOR TESTING PURPOSES ONLY !!!")
                ...
                if not self.flashbots_provider:
                    logger.error("Flashbots provider not initialized")
                    return {"included": False, "status": "error", "error": "Flashbots provider not initialized"}
                ...
                if not await self.flashbots_provider.is_initialized():
                    try:
                        ...
                        logger.error(f"Failed to initialize Flashbots provider: {str(e)}")
                        return {"included": False, "status": "error", "error": f"Failed to initialize Flashbots provider: {str(e)}"}
                
                # Wait for bundle to be included
                try:
                    inclusion_result = await self.flashbots_provider.wait_for_bundle(bundle_hash)
                    
                    if inclusion_result and inclusion_result.get("included"):
                        logger.info(f"Bundle {bundle_hash} included in block {inclusion_result.get('block_number')}")
                        return {
                            "included": True,
                            "transaction_hash": inclusion_result.get("transaction_hash"),
                            "block_number": inclusion_result.get("block_number"),
                        ...
                        logger.warning(f"Bundle {bundle_hash} not included: {inclusion_result}")
                        return {"included": False, "status": "not_included", "error": "Bundle not included"}
                
                except Exception as e:
                    logger.error(f"Failed to wait for bundle inclusion: {str(e)}")
                    return {"included": False, "status": "error", "error": f"Failed to wait for bundle inclusion: {str(e)}"}
        except Exception as e:
