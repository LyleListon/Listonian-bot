...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("logs/arbitrage.log")],
)
...
class ProductionRunner:
    ...
    async def start(self):
        ...
        try:
            ...
            token_addresses = [
                info["address"] for info in self.config["tokens"].values()
            ]
            for dex_name in self.executor.path_finder.dex_manager.dexes:
                ...
                await self.executor.path_finder.dex_manager.discover_pools(
                    dex_name=dex_name, token_addresses=token_addresses
                )
                ...
                try:
                    ...
                    for token in tokens:
                        ...
                        result = await self.executor.find_and_execute_arbitrage(
                            token_address=token,
                            amount_wei=self.config.get(
                                "scan_amount_wei", 1000000000000000000
                            ),  # 1 ETH default
