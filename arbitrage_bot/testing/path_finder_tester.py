...
import os
import csv
from typing import Dict, List, Any, Optional, Tuple # Removed Set
...
from ..core.gas.gas_optimizer import GasOptimizer, create_gas_optimizer
...
class PathFinderTester:
    ...
    def __init__(
        ...
        self.test_amounts = self.test_config.get(
            "test_amounts", [0.1, 1.0, 5.0]  # 0.1 ETH  # 1 ETH  # 5 ETH
        )
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if not self.dex_manager:
                self.dex_manager = await DexManager(
                    self.web3_manager, self.config
                ).initialize()

            # Initialize GasOptimizer
            # Call the imported factory function directly
            self.gas_optimizer = await create_gas_optimizer(
                self.dex_manager, self.web3_manager, self.config
            )
    ...
    async def discover_test_tokens(self) -> List[str]:
        ...
        try:
                    ...
                    if hasattr(dex, "get_supported_tokens"):
                        ...
                        for token in tokens:
                            if token not in discovered_tokens:
                                ...
                                for (
                                    check_dex
                                ) in self.dex_manager.dex_instances.values():
                                    ...
                                    try:
                                        if hasattr(
                                            check_dex, "_get_pool_liquidity_by_tokens"
                                        ):
                                            liquidity = await check_dex._get_pool_liquidity_by_tokens(
                                                token, check_dex.weth_address
                                            )
                                        ...
                                        logger.debug(
                                            f"Error checking liquidity for {token}: {e}"
                                        )

                                if has_liquidity:
                                    discovered_tokens.append(token)
                                    if (
                                        len(discovered_tokens) >= 10
                                    ):  # Limit discovery to 10 tokens
                                        break
                except Exception as e:
            ...
            if discovered_tokens:
                logger.info(
                    f"Discovered {len(discovered_tokens)} tokens with sufficient liquidity"
                )
                return discovered_tokens

            # Fallback to common tokens
            logger.warning(
                "Could not discover tokens with sufficient liquidity, using fallback tokens"
            )
            return [
    ...
    async def run_test_case(self, test_case: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
                    ...
                    for i in range(len(best_path.dexes)):
                        ...
                        if len(hop_tokens) == 2:
                            if hasattr(dex, "estimate_gas"):
                                hop_gas += await dex.estimate_gas(
                                    hop_tokens[0], hop_tokens[1]
                                )
                            else:
                                hop_gas += 150000  # Default estimate

                    # Compare with path finder's estimate
                    gas_accuracy = abs(estimated_gas - hop_gas) / max(
                        estimated_gas, hop_gas
                    )
                    gas_accuracy = 1.0 - min(
                        gas_accuracy, 1.0
            ...
            logger.info(
                f"Test case {description} completed: {'success' if success else 'no paths found'}"
            )
            return result
    ...
    async def run_test_batch(
        ...
        for i in range(0, len(test_cases), self.batch_size):
            ...
            batch_results = await asyncio.gather(
                *[self.run_test_case(case) for case in batch]
            )
    ...
    async def run_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            success_rate = (
                sum(self.success_rates) / len(self.success_rates)
                if self.success_rates
                else 0
            )
            avg_gas_accuracy = (
                sum(self.gas_estimation_accuracy) / len(self.gas_estimation_accuracy)
                if self.gas_estimation_accuracy
                else 0
            )
            ...
            logger.info(
                f"Tests completed: {summary['success_count']}/{summary['test_count']} successful ({summary['success_rate']*100:.1f}%)"
            )
            return summary
    ...
    async def save_results(self, summary: Dict[str, Any]) -> None:
        ...
        try:
            ...
            with open(json_path, "w") as f:
                json.dump(
                    {"summary": summary, "results": self.test_results}, f, indent=2
                )
...
async def run_path_finder_tests():
    ...
    parser.add_argument(
        "--batch-size", type=int, default=5, help="Batch size for test execution"
    )
    parser.add_argument(
        "--max-tests", type=int, default=20, help="Maximum number of test cases to run"
    )
    ...
    # Load config
    config = None
    # Always load config using the standard function
    from ..utils.config_loader import load_config
    config = load_config()
    # Note: The --config CLI argument is now ignored as load_config_from_file doesn't exist.
    ...
    tester = PathFinderTester(config=config, output_dir=args.output_dir)
    ...
    if "error" in summary:
        ...
        logger.info(
            f"  Average execution time: {summary['avg_execution_time']*1000:.1f}ms"
        )
        logger.info(
            f"  Gas estimation accuracy: {summary['avg_gas_accuracy']*100:.1f}%"
        )
