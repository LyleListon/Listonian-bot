...
def setup_logging(log_level=logging.DEBUG):  # Changed default to DEBUG
    ...
    file_handler = logging.handlers.RotatingFileHandler(
        general_log, maxBytes=10 * 1024 * 1024, backupCount=5  # 10MB
    )
    ...
    metrics_handler = logging.handlers.RotatingFileHandler(
        metrics_log, maxBytes=10 * 1024 * 1024, backupCount=5  # 10MB
    )
    ...
    opportunity_handler = logging.handlers.RotatingFileHandler(
        opportunity_log, maxBytes=10 * 1024 * 1024, backupCount=5  # 10MB
    )
    ...
    execution_handler = logging.handlers.RotatingFileHandler(
        execution_log, maxBytes=10 * 1024 * 1024, backupCount=5  # 10MB
    )
...
def log_system_metrics(
    memory_usage: float, cpu_usage: float, thread_count: int, active_approvals: int
):
