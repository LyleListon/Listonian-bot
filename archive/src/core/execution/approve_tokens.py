...
async def approve_token_for_dex(
    ...
    try:
        ...
        max_fee_per_gas = int((base_fee_per_gas + max_priority_fee_per_gas) * 1.5)  # Higher multiplier
        ...
        try:
            ...
            logger.info(f"Transaction parameters: {json.dumps(approve_tx, indent=2, default=str)}")
            ...
            tx_hash = await web3_manager.w3.eth.send_raw_transaction(signed_tx.raw_transaction)
            ...
            if receipt['status'] == 1:
                logger.info(f"Successfully approved token {token_address} for {router_address}")
                return True
            else:
            ...
            if "replacement transaction underpriced" in str(e):
                ...
                logger.warning(f"Transaction underpriced, retrying with higher gas price")
            ...
            logger.error(f"Transaction parameters: {json.dumps(approve_tx, indent=2, default=str)}")
            return False
