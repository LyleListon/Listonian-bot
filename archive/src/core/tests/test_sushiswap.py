...
async def test_initialization_with_invalid_router(web3_manager, dex_configs):
    ...
    web3_manager.w3 = create_mock_web3(
        factory_error=BadFunctionCallOutput()
    )
...
async def test_get_quote_with_impact(sushiswap_dex, token_addresses, test_amounts):
    ...
    with patch.object(BaseDEXV2, 'get_quote_with_impact', new_callable=AsyncMock) as mock_quote:
...
async def test_error_handling(sushiswap_dex):
    ...
    with patch.object(BaseDEXV2, 'get_quote_with_impact', side_effect=ValueError("Invalid path")):
        with pytest.raises(ValueError, match="Sushiswap quote calculation failed: Invalid path"):
