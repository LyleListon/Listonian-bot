...
class WebSocketServer:
    ...
    async def get_dex_status(self) -> list:
        ...
        # Default DEX list when no metrics available
        default_dexes = {
            'baseswap': 'BaseSwap',
        ...
        if metrics and len(metrics) > 0 and 'dex_metrics' in metrics[0]:
            dex_metrics = metrics[0]['dex_metrics']
        else:
            # Create empty metrics for default DEXes
            dex_metrics = {name: {} for name in default_dexes.keys()}

        for dex_name, dex_data in dex_metrics.items():
            # Format DEX name for display
            display_name = dex_name.replace('_', ' ').title()
            if dex_name.lower() in default_dexes:
                display_name = default_dexes[dex_name.lower()]
            elif dex_name.lower() == 'uniswap_v3':
            ...
            dexes.append({
                'name': display_name,
                'active': dex_data.get('active', True),  # Assume active by default
                'liquidity': dex_data.get('liquidity', 0),
                'volume_24h': dex_data.get('volume_24h', 0)
            })

        logger.debug(f"DEX status: {dexes}")
        return dexes
    ...
    async def get_arbitrage_opportunities(self) -> list:
        ...
        # Default token pairs to show
        default_pairs = ['WETH', 'USDC', 'USDT']
        opportunities = []
        
        try:
            if hasattr(self.analytics_system, 'market_conditions'):
                market_conditions = self.analytics_system.market_conditions
            else:
                market_conditions = {}

            if market_conditions:
                # Use actual market conditions
                for symbol, condition in market_conditions.items():
                    if condition.price_impact < 0.01:
                ...
                for token in default_pairs:
                    opportunities.append({
                        'token_pair': f"{token}/USDC",
                        'potential_profit': 0.0,
                        'gas_cost': 0.0,
                        'net_profit': 0.0,
                        'price_impact': 0.005,  # Default 0.5% impact
                        'executable': True
                    })
        except Exception as e:
            logger.error(f"Error getting arbitrage opportunities: {e}")
            # Return default opportunities on error
            for token in default_pairs:
                opportunities.append({
                    'token_pair': f"{token}/USDC",
                    'potential_profit': 0.0,
        ...
        try:
            ...
            if 'metrics' in channels and self.analytics_system:
                try:
                    ...
                    metrics = [{}]  # Empty metrics but valid structure

                # Use default values if no metrics available
                latest = metrics[0] if metrics and len(metrics) > 0 else {
                    'total_trades': 0,
                    'trades_24h': 0,
                    'success_rate': 0.0,
                ...
                logger.info(f"Sent initial metrics: {data}")
            ...
            if 'performance' in channels and self.analytics_system:
                ...
                await websocket.send(json.dumps(data))
                logger.info(f"Sent initial performance: {data}")
