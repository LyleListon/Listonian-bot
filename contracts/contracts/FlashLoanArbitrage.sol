...
contract FlashLoanArbitrage {
    ...
    event ProfitTransferred(address token, uint256 amount, address recipient);
    event ProfitRecipientUpdated(address oldRecipient, address newRecipient);

    constructor(address _addressProvider) {
        owner = msg.sender;
        profitRecipient = msg.sender; // Initially set to owner
    ...
    function setProfitRecipient(address _profitRecipient) external onlyOwner {
        ...
        emit ProfitRecipientUpdated(profitRecipient, _profitRecipient);
    ...
    function executeOperation(
        ...
        // Transfer profit to recipient immediately
        if (profit > 0) {
            IERC20(asset).safeTransfer(profitRecipient, profit);
            emit ProfitTransferred(asset, profit, profitRecipient);
        }
    ...
    function executeV3Trade(address asset, address router, uint256 amount) internal virtual {
        // Approve router
    ...
    function executeV2Trade(address asset, address router, uint256 amount) internal virtual {
        // Approve router
        IERC20(asset).approve(router, amount);
