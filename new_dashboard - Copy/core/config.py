...
class DashboardSettings(BaseSettings):
    ...
    memory_bank_path: Path = Field(
        ...
        description="Path to memory bank storage"
    )
    
    # Web3 settings
    rpc_url: str = Field(
        default="",
        description="RPC URL for Web3 connection"
    )
    chain_id: int = Field(
        default=8453,  # Base chain
        description="Chain ID for Web3 connection"
    )
    ...
    max_metrics_age: int = Field(
        default=300,
        description="Maximum age in seconds for stored metrics"
    )
