...
class ArbitrageConnector:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            config_paths = ["configs/production.json", "configs/config.json"]
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        "PathFinder module not found, some functions may be limited"
                    )

                # Try to import flash loan manager if available
                try:
                    from arbitrage_bot.core.flash_loan_manager_async import (
                        AsyncFlashLoanManager,
                    )

                    self.flash_loan_manager = AsyncFlashLoanManager(
                        self.web3_manager, self.config
                    )
                except ImportError:
                    logger.warning(
                        "FlashLoanManager module not found, some functions may be limited"
                    )
    ...
    def get_connection_status(self) -> Dict[str, Any]:
        ...
        if self.dex_manager:
            status["dexes_loaded"] = (
                len(self.dex_manager.dexes) if hasattr(self.dex_manager, "dexes") else 0
            )
    ...
    def execute_flash_loan_arbitrage(
        self, path: List[str], amount: float
    ) -> Dict[str, Any]:
    ...
    def manual_trade(
        self, buy_dex: str, sell_dex: str, token_address: str, amount: float
    ) -> Dict[str, Any]:
    ...
    def get_wallet_balance(self, address: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "eth_balance": balance,
                "usd_value": (
                    self.web3_manager.get_eth_to_usd(balance)
                    if hasattr(self.web3_manager, "get_eth_to_usd")
                    else None
                ),
            }
        except Exception as e:
    ...
    def get_gas_settings(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "max_fee_per_gas": (
                    self.web3_manager.get_max_fee_per_gas()
                    if hasattr(self.web3_manager, "get_max_fee_per_gas")
                    else None
                ),
                "max_priority_fee": (
                    self.web3_manager.get_max_priority_fee()
                    if hasattr(self.web3_manager, "get_max_priority_fee")
                    else None
                ),
                "gas_limit_multiplier": self.config.get("gas", {}).get(
