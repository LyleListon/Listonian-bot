...
class ServiceRegistry:
    ...
    async def initialize(self) -> None:
        ...
        async with self._lock:
            ...
            self.logger.info("=== Initializing service registry with %d services ===", len(self._services))
            for name, service in self._services.items():
                ...
                if init_method_name:
                    ...
                    self.logger.info("About to call %s() on service: %s (type: %s)", 
                                    init_method_name, name, type(service).__name__)
                    try:
                        if asyncio.iscoroutinefunction(init_method):
                            self.logger.info("Calling async %s() on service: %s", init_method_name, name)
                            await init_method()
                        else:
                            self.logger.info("Calling sync %s() on service: %s", init_method_name, name)
                            init_method()
                        self.logger.info("Successfully called %s() on service: %s", init_method_name, name)
                    except Exception as e:
                        self.logger.error("Failed to call %s() on service %s: %s", init_method_name, name, str(e), exc_info=True)
                        raise
...
def register_services() -> None:
    ...
    try:
        logger.info("About to register MemoryService with registry type: %s", type(registry))
        ...
        logger.info("About to register MetricsService with memory_service type: %s", type(memory_service))
        ...
        logger.info("About to register SystemService with memory_service and metrics_service")
