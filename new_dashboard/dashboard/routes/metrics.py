...
from ..core.dependencies import get_metrics_service, get_market_data_service # Keep market_data_service import for other routes
...
async def get_current_metrics(
    metrics_service=Depends(get_metrics_service) # Remove unused market_data_service dependency
) -> Dict[str, Any]:
...
async def get_performance_metrics(
    ...
    market_data = await market_data_service.get_current_market_data() # Keep for now, might be redundant
    ...
    performance = {
        "performance": {
            ...
            "previous_gas_efficiency": metrics.get("metrics", {}).get("profit_per_gas", 0.0), # Use calculated profit_per_gas
            "profit_distribution": metrics.get("metrics", {}).get("profit_distribution_by_strategy", {}), # Use calculated profit_distribution_by_strategy
            "success_trend": [], # Placeholder - Needs trend calculation logic
            "transactions": metrics.get("trade_history", []) # Use trade history from metrics
        },
        "opportunities": {
             # Check if this path is correct in the final metrics structure
            "volume_trend": metrics.get("market_data", {}).get("analysis", {}).get("opportunity_timing", {}).get("hourly_distribution", {})
        },
