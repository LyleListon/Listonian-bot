...
async def detailed_health_check(
    ...
    return {
        ...
        "network": network,
        "issues": (
            {"critical_components": critical_failures, "network": not is_connected}
            if critical_failures or not is_connected
            else None
        ),
...
@router.get("/uptime")
@log_execution_time
async def get_uptime(system_service=Depends(get_system_service)) -> Dict[str, Any]:
