...
class MemoryService:
    ...
    async def initialize(self):
        ...
        try:
            logger.info(
                "Setting storage directory to absolute path: %s",
                self.base_dir.absolute(),
            )
    ...
    async def _load_state(self):
        ...
        try:
            async with self._lock:
                ...
                logger.debug(
                    f"Loaded state: {json.dumps(self._current_state, indent=2)}"
                )
        ...
        async with self._lock:
            ...
            if "trade_history" in updates:
                await self.file_manager.write_file(
                    "trades", {"trades": self._current_state["trade_history"]}
                )
            if "metrics" in updates:
                await self.file_manager.write_file(
