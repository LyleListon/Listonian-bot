...
class MetricsService:
    ...
    async def _load_metrics(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded initial state from memory service: {json.dumps(state)}"
            )
            await self._handle_memory_update(
                state
            )  # Use the handler to populate initial state
            logger.info("Initial metrics loaded from memory service.")
            # Log the current metrics structure after processing
            logger.info(
                f"Current metrics structure after processing: {json.dumps(self._current_metrics)}"
            )
        except Exception as e:
            logger.error(f"Error loading initial metrics: {e}")
            # Log the actual structure of the metrics file
            try:
                metrics_data = await self.memory_service.file_manager.read_file(
                    "metrics"
                )
    ...
    async def _handle_memory_update(self, update: Dict[str, Any]):
        ...
        async with self._lock:
            ...
            logger.info(
                f"Received memory_metrics keys: {list(memory_metrics_received.keys())}"
            )

            logger.info(
                f"Received memory_metrics content: {json.dumps(memory_metrics_received)}"
            )
            ...
            current_metrics_stats.update(
                {
                    "total_profit_eth": memory_metrics.get("profitability", {}).get(
                        "total_profit",
                        current_metrics_stats.get("total_profit_eth", 0.0),
                    ),
                    "success_rate": memory_metrics.get("dex_performance", {})
                    .get("success_rates", {})
                    .get("overall", current_metrics_stats.get("success_rate", 0.0)),
                    "total_trades": memory_metrics.get("profitability", {}).get(
                        "total_trades", current_metrics_stats.get("total_trades", 0)
                    ),
                    "gas_price": memory_metrics.get("execution", {})
                    .get("gas_by_strategy", {})
                    .get("average", current_metrics_stats.get("gas_price", 0.0)),
                }
            )

            # Update market data if present in the update
            if "market_data" in memory_metrics:
                self._stats["market_data"] = memory_metrics.get(
                    "market_data", self._stats.get("market_data", {})
                )

            # Update trade history
            self._stats["trade_history"] = update.get(
            ...
            roi_by_trade_type = defaultdict(
                lambda: {"total_roi": decimal.Decimal(0), "count": 0}
            )
            ...
            for trade in trade_history:
                ...
                profit = decimal.Decimal(
                    str(trade.get("profit", 0.0))
                )  # Assuming profit is in ETH or similar base unit
                gas_cost = decimal.Decimal(
                    str(trade.get("gas_cost", 0.0))
                )  # Assuming gas_cost is in the same unit
                token_pair = trade.get("token_pair", "Unknown")  # e.g., "ETH/USDC"
                trade_type = trade.get(
                    "trade_type", "unknown"
                )  # e.g., "simple", "flashloan"
                strategy_type = trade.get(
                    "strategy_type", "unknown"
                )  # e.g., "cross_dex", "triangular"
                initial_investment = decimal.Decimal(
                    str(trade.get("initial_investment", 0.0))
                ...
                profit_by_strategy[
                    strategy_type
                ] += net_profit  # Using net profit for distribution

            # Calculate final aggregate metrics
            # 3. Profit per gas unit
            profit_per_gas = (
                total_net_profit / total_gas_cost
                if total_gas_cost > 0
            ...
            if total_net_profit > 0:
                for strategy, profit_sum in profit_by_strategy.items():
                    ...
                    profit_distribution_by_strategy[strategy] = float(
                        distribution
            ...
            for trade_type, data in roi_by_trade_type.items():
                if data["count"] > 0:
                    ...
                    avg_roi_by_trade_type[trade_type] = float(
                        avg_roi
                    )  # Store as float for JSON
            ...
            current_metrics_stats.update(
                {
                    ...
                    "net_profit_by_pair": {
                        k: float(v) for k, v in net_profit_by_pair.items()
                    },
            ...
            while True:
                try:
                    async with self._lock:
                        ...
                        self._current_metrics["system_status"] = self._stats[
                            "system_status"
                        ]
                        self._current_metrics["timestamp"] = self._stats[
                            "timestamp"
                        ]  # Use timestamp from memory update
            ...
            for queue in active_subscribers:
                if queue in self._subscribers:  # Check if still subscribed
                    try:
                        ...
                        logger.warning(
                            "Subscriber queue full or unresponsive, skipping."
                        )
                    except Exception as e:
        ...
        try:
            ...
            async with self._lock:
                ...
                if len(self._price_history[dex]) > self._max_price_history:
                    self._price_history[dex] = self._price_history[dex][
                        -self._max_price_history :
                    ]
                ...
                if len(self._price_history[dex]) >= 5:  # Need at least 5 data points
                    recent_changes = [
                        record["change_pct"]
                        for record in self._price_history[dex][-10:]
                    ]
                    try:
                ...
                self._stats["market_data"]["analysis"]["volatility"][dex] = (
                    self._price_volatility.get(dex, 0)
                )
                ...
                if len(self._stats["market_data"]["price_updates"][dex]) > 20:
                    self._stats["market_data"]["price_updates"][dex] = self._stats[
                        "market_data"
                    ]["price_updates"][dex][-20:]
                ...
                logger.info(
                    f"Price update for {dex} processed successfully. New price: {new_price}, Volatility: {self._price_volatility.get(dex, 0):.4f}%"
                )
        except Exception as e:
