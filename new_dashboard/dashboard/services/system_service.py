...
class SystemService:
            ...
            while True:
                try:
                    async with self._lock:
                        ...
                        await self.memory_service.update_state({"system_metrics": self._detailed_metrics})
    ...
    async def _collect_detailed_metrics(self):
        ...
        try:
            # CPU metrics
            self._detailed_metrics["cpu"]["usage_percent"] = psutil.cpu_percent(interval=0.1)
            self._detailed_metrics["cpu"]["per_core"] = psutil.cpu_percent(interval=0.1, percpu=True)
            
            # Load average (on Unix systems)
            try:
                self._detailed_metrics["cpu"]["load_avg"] = [x / psutil.cpu_count() * 100 for x in psutil.getloadavg()]
            ...
            self._detailed_metrics["system"]["uptime"] = time.time() - psutil.boot_time()
