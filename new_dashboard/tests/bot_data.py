...
def get_arbitrage_opportunities(max_entries: int = 10) -> List[Dict[str, Any]]:
    ...
    log_entries = get_recent_log_entries(
        max_entries=500
    )  # Get more entries to find opportunities
...
def get_profit_summary() -> Dict[str, Any]:
    ...
    if summary["profitable_trades"] > 0:
        summary["average_profit"] = (
            summary["total_profit_eth"] / summary["profitable_trades"]
        )
...
def get_dex_status() -> Dict[str, Dict[str, Any]]:
    ...
    for dex_name, entries in dex_entries.items():
        # Default status
        status = {"status": "Unknown", "last_activity": "Unknown", "errors": 0}
...
def get_gas_statistics() -> Dict[str, Any]:
    ...
    if stats["transactions_count"] > 0:
        stats["average_gas_used"] = (
            stats["total_gas_used"] // stats["transactions_count"]
        )
