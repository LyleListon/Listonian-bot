...
class SwapBasedDEX(BaseDEX):
    ...
    def __init__(self, web3_manager: Web3Manager, config: Dict[str, Any]):
        ...
        self.min_priority_fee = float(config.get('flashbots', {}).get('min_priority_fee', '1.5'))
        self.max_priority_fee = float(config.get('flashbots', {}).get('max_priority_fee', '3'))
        self.sandwich_detection = config.get('flashbots', {}).get('sandwich_detection', True)
        self.frontrun_detection = config.get('flashbots', {}).get('frontrun_detection', True)
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if self.flashbots_enabled:
                try:
                    auth_key = self.web3_manager.config.get('flashbots', {}).get('auth_signer_key')
                    if not auth_key:
                        logger.warning("Flashbots auth key not found, disabling Flashbots")
                        self.flashbots_enabled = False
                    else:
                        self.flashbots = self.web3_manager.get_flashbots_client(auth_key)
    ...
    async def get_quote_with_impact(
        ...
        try:
            ...
            if self.flashbots_enabled and (self.sandwich_detection or self.frontrun_detection):
                try:
    ...
    async def swap_exact_tokens_for_tokens(
        ...
        try:
            ...
            if self.flashbots_enabled and self.adaptive_gas:
                ...
                gas_price = gas_price if gas_price else await self.web3_manager.get_max_fee()
                priority_fee = await self.web3_manager.get_priority_fee()
